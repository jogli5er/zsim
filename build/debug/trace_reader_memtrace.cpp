...
void TraceReaderMemtrace::binaryGroupPathIs(const std::string &_path) {
  ...
  if (!_path.empty()) {
    ...
    std::string error =
        directory_.initialize_module_file(_path + "/modules.log");
    if (!error.empty()) {
      panic(
    ...
    module_mapper_ =
        module_mapper_t::create(directory_.modfile_bytes_,
#ifdef ZSIM_USE_YT
...
void TraceReaderMemtrace::processInst(InstInfo *_info) {
  ...
  tie(mt_mem_ops_, unknown_type, cond_branch, std::ignore, std::ignore) =
      xed_tuple;
