...
class Cache : public BaseCache {
 protected:
  ...
  // Latencies
  uint32_t accLat;  // latency of a normal access (could split in get/put,
                    // probably not needed)
 ...
 public:
  Cache(uint32_t _numLines, CC* _cc, CacheArray* _array, ReplPolicy* _rp,
        uint32_t _accLat, uint32_t _invLat, const g_string& _name);

  const char* getName() override;
  void setParents(uint32_t _childId, const g_vector<MemObject*>& parents,
                  Network* network) override;
  g_vector<MemObject*>* getParents() override { return cc->getParents(); }
  void setChildren(const g_vector<BaseCache*>& children,
                   Network* network) override;
 ...
 protected:
  ...
  uint64_t finishInvalidate(
      const InvReq& req);  // performs inv and releases downLock
};
