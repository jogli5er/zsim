 ...
 * 2) High-performance stats: Updating counters should be as fast as updating
 * raw integers. Counters are objects though, so they entail some space
 * overhead. 3) Allow fixed-size stats output: The stat types supported are all
 * fixed-size, and stats cannot be created after initialization. This allows
 * fixed-size records, making periodic stats much easier to parse and **iterate
 * over** (e.g. we can parse 1% of the samples for a high-level graph without
 * bringing the whole stats file from disk, then zoom in on a specific portion,
 * etc.).
 *
...
/* TODO: I want these to be POD types, but polymorphism (needed by dynamic_cast)
 * probably disables it. Dang. */
...
class AggregateStat : public Stat {
 ...
 public:
  /* An aggregate stat is regular if all its children are 1) aggregate and 2) of
   * the same type (e.g. all the threads). This lets us express all the subtypes
   * of instances of a common datatype, and this collection as an array. It is
   * useful with HDF5, where we would otherwise be forced to have huge compund
   * datatypes, which HDF5 can't do after some point.
   */
  explicit AggregateStat(bool isRegular = false)
      : Stat(), _isMutable(true), _isRegular(isRegular) {}
  ...
  // Returns true if it is a non-empty type, false otherwise. Empty types are
  // culled by the parent.
  bool makeImmutable() {
    assert(_isMutable);
  ...
  Stat* get(uint32_t idx) const { return _children[idx]; }

  // Access-while-mutable interface
  uint32_t curSize() const { return _children.size(); }
};
...
class VectorStat : public Stat {
 ...
 public:
  ...
  inline bool hasCounterNames() { return (_counterNames != nullptr); }
...
class Counter : public ScalarStat {
 ...
 public:
  ...
  inline void inc(uint64_t delta) { _count += delta; }
  ...
  inline void atomicInc() { __sync_fetch_and_add(&_count, 1); }

  uint64_t get() const { return _count; }

  inline void set(uint64_t data) { _count = data; }
};
...
class VectorCounter : public VectorStat {
 ...
 public:
  ...
  inline void inc(uint32_t idx, uint64_t value) { _counters[idx] += value; }

  inline void inc(uint32_t idx) { _counters[idx]++; }
  ...
  inline virtual uint64_t count(uint32_t idx) const { return _counters[idx]; }

  inline uint32_t size() const { return _counters.size(); }

  inline void set(uint32_t idx, uint64_t data) { _counters[idx] = data; }
};
...
class ProxyStat : public ScalarStat {
 ...
 public:
  ...
  uint64_t get() const {
    assert(_statPtr);  // TODO: we may want to make this work only with
                       // volatiles...
    return *_statPtr;
  }
};
 ...
 * If your stat depends on a formula, this lets you encode it compactly using
 * C++11 lambdas
 *
 * Usage example:
 *  auto x = [this]() { return curCycle - haltedCycles; }; //declare the lambda
 * function that computes the stat; note this is captured because these values
 * are class members LambdaStat<decltype(x)>* cyclesStat = new
 * LambdaStat<decltype(x)>(x); //instantiate the templated stat. Each lambda has
 * a unique type, which you get with decltype cyclesStat->init("cycles",
 * "Simulated cycles"); //etc. Use as an usual stat!
 */
template <typename F>
...
class LambdaVectorStat : public VectorStat {
 ...
 public:
  ...
  uint64_t count(
      uint32_t idx) const {  // dsm: Interestingly, this compiles even if f() is
                             // not const. gcc may catch this eventually...
    assert(idx < s);
    return f(idx);
  }
};
...
template <typename F>
LambdaStat<F>* makeLambdaStat(F f) {
  return new LambdaStat<F>(f);
}

template <typename F>
LambdaVectorStat<F>* makeLambdaVectorStat(F f, uint32_t size) {
  return new LambdaVectorStat<F>(f, size);
}
...
class CsvBackendImpl;

class CsvBackend : public StatsBackend {
 private:
  CsvBackendImpl* backend;

 public:
  CsvBackend(const char* filename, AggregateStat* rootStat);
  virtual void dump(bool buffered);
};
...
class HDF5Backend : public StatsBackend {
 ...
 public:
  HDF5Backend(const char* filename, AggregateStat* rootStat,
              size_t bytesPerWrite, bool skipVectors,
