...
struct bbl_cache_entry {
  ...
  bbl_cache_entry(BblInfo *_bbl, uint32_t _size_bytes, uint32_t _nr_inst,
                  uint32_t _mem_ops)
      : bbl(_bbl),
...
struct threadInfo {
  ...
  threadInfo(int _tid, std::string _tracefile, std::string _binaries,
             std::string _type)
...
void sim_mem_op(uint32_t tid, const InstInfo *insi) {
  if (!INS_Nop(insi->ins) &&
      !INS_LEA(insi->ins)) {  // NOPs and LEA never access memory
    for (uint32_t op = 0;
...
void *simtrace(void *arg) {
  ...
  while (!finished) {
    ...
    if (bbl_to_sim == bbl_cache.end()) {
      ...
      /* In Memtrace, basic blocks can be interrupted by signal handlers and
      ...
      while (1) {
        ...
        /* This Bbl contains unknown instructions (NOPs). We can simulate but
         * not cache */
        ...
        /* This Bbl was interrrupted by a context switch. Do not simulate and do
         * not cache */
      ...
      for (auto it = bbl.begin(); it != bbl.end(); it++, cur_ins++) {
        ...
        if (info.mem_used[0] ||
            info.mem_used[1]) {  // Expect idx 1 => idx 0, but short circuit is
                                 // fast
          sim_mem_op(tid, &info);
        }
  ...
  info(
      "Tid: %i finished. Instruction trace size: %lu. Skipped instructions due "
      "to unavailable symbols: %lu. BBLs in trace: %lu, dropped BBLs due to "
      "context switches: %lu",
...
int main(int argc, char *argv[]) {
  ...
  /* Remove all zsim.log.* files (we append to them, and want to avoid outputs
  ...
  for (uint32_t i = 0; i < zinfo->numCores; i++) {
    ...
    if (conf.exists(p_ss.str().c_str())) {
      const char *trace =
          realpath(conf.get<const char *>(p_ss.str(), cwd), nullptr);
      const char *binaries =
          realpath(conf.get<const char *>("trace_binaries", cwd), nullptr);
      const char *type = conf.get<const char *>("trace_type", cwd);

      ti[i] = {(int32_t)i, std::string(trace), std::string(binaries),
               std::string(type)};
