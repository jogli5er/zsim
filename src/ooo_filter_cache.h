...
class OOOFilterCache : public FilterCache {
 ...
 public:
  OOOFilterCache(uint32_t _numSets, uint32_t _numLines, CC* _cc,
      ...
      : FilterCache(_numSets, _numLines, _cc, _array, _rp, _accLat, _invLat,
                    _name) {
    numLinesNLP = 0;
  ...
  explicit OOOFilterCache(uint32_t _numSets, uint32_t _numLines, CC* _cc,
      ...
      : OOOFilterCache(_numSets, _numLines, _cc, _array, _rp, _accLat, _invLat,
                       _name) {
  ...
  explicit OOOFilterCache(uint32_t _numSets, uint32_t _numLines, CC* _cc,
    ...
    if (pref_degree) {
      dataflow_prefetcher = new DataflowPrefetcher(
          "dataflow_" + _name, _pref_degree, _name, true, false, _pref_constant,
          _pref_complex, _limit_prefetching, _var_degree, this,
          _pref_kernels_file);
    }
  }
#endif

  inline uint64_t load(Address vAddr, uint64_t curCycle, uint64_t dispatchCycle,
                       Address pc, OOOCoreRecorder* cRec, uint8_t size) {
    Address vLineAddr = vAddr >> lineBits;
    // L1 latency as returned by load() is zero, hence add accLat
    uint64_t respCycle = FilterCache::load(vAddr, dispatchCycle, pc, size);
