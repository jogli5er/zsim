...
class DataflowPrefetcher : public CachePrefetcher {
 ...
 private:
  ...
  Counter prof_emitted_prefetches, prof_emitted_actual_prefetches,
      prof_prefetcher_accessed, prof_kernels_found, prof_wrong_path_prefetches,
      prof_pc_not_found, prof_prefetcher_throttled;
 ...
 public:
  explicit DataflowPrefetcher(const g_string& _name, uint32_t _degree,
                              const g_string& target_cache, bool _monitor_GETS,
                              bool _monitor_GETX, bool _pref_simple,
                              bool _pref_complex, bool _limit_prefetching,
                              bool _var_degree, FilterCache* _filter_cache,
                              g_string _pref_kernels_file)
      : CachePrefetcher(_name, target_cache),
        degree_(_degree),
        monitor_GETS(_monitor_GETS),
        monitor_GETX(_monitor_GETX),
        pref_simple(_pref_simple),
        pref_complex(_pref_complex),
        limit_prefetching(_limit_prefetching),
        variable_degree(_var_degree),
        filter_cache(_filter_cache),
        pref_kernels_file(_pref_kernels_file) {
    if (_degree) {
      initKernels();
    }
  };
