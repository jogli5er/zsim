 ...
 * - Strict config: type errors, warnings on unused variables, panic on
 * different defaults
 * - Produce a full configuration file with all the variables, including
 * defaults (for config parsing, comparison, etc.)
 */
...
class Config {
 ...
 public:
  ...
  // Called when initialization ends. Writes output config, and emits warnings
  // for unused input settings
  ...
  // Access interface
  // T can be uint32_t, uint64_t, bool, or const char*. Instantiations are in
  // the cpp file
 ...
 private:
  template <typename T>
  T genericGet(const char* key);
  template <typename T>
  T genericGet(const char* key, T def);
  template <typename T>
  std::vector<T> genericGetArray(const char* key);
};
...
/* Parses a delimiter-separated list of T's (typically ints, see/add
 * specializtions in .cpp) 0-elem lists are OK panics on parsing and
...
template <typename T>
std::vector<T> ParseList(const std::string& listStr, uint32_t maxSize,
                         uint32_t fillValue) {
