...
#define INS_Nop(ins)                        \
  (INS_Category(ins) == XED_CATEGORY_NOP || \
   INS_Category(ins) == XED_CATEGORY_WIDENOP)
...
#define INS_OperandMemoryIndexReg(ins, op) \
  xed_decoded_inst_get_index_reg(ins, op)
#define INS_LockPrefix(ins) \
  xed_decoded_inst_get_attribute(ins, XED_ATTRIBUTE_LOCKED)
...
typedef xed_category_enum_t INS_CAT;
/* Just like PIN we break BBLs on a number of additional instructions such as
 * REP */
#define INS_ChangeControlFlow(ins)                                           \
  (INS_Category(ins) == XC(COND_BR) || INS_Category(ins) == XC(UNCOND_BR) || \
   INS_Category(ins) == XC(CALL) || INS_Category(ins) == XC(RET) ||          \
   INS_Category(ins) == XC(SYSCALL) || INS_Category(ins) == XC(SYSRET) ||    \
   INS_Opcode(ins) == XO(CPUID) || INS_Opcode(ins) == XO(POPF) ||            \
   INS_Opcode(ins) == XO(POPFD) || INS_Opcode(ins) == XO(POPFQ) ||           \
   INS_IsRep(ins))
...
static inline void *wrap_thread_func(void *fa) {
  std::pair<void (*)(void *), void *> *p =
      (std::pair<void (*)(void *), void *> *)fa;

  p->first(p->second);
  return NULL;
}
#define PIN_SpawnInternalThread(func, arg, stackSize, threadid) \
  pthread_create(                                               \
      &contention_thread, NULL, &wrap_thread_func,              \
      (void *)(new std::pair<void (*)(void *), void *>(&func, arg)));
