 ...
 *  - L1: Branch history shift registers (bshr): 2^NB entries, HB bits of
 * history/entry, indexed by XOR'd PC
 *  - L2: Pattern history table (pht): 2^LB entries, 2-bit sat counters, indexed
 * by XOR'd bshr contents NOTE: Assumes LB is in [NB, HB] range for XORing
 * (e.g., HB = 18 and NB = 10, LB = 13 is OK)
 */
class BranchPredictorPAg {
 ...
 public:
  ...
  inline bool predict(Address branchPc, bool taken) {
    ...
    // Predict
    // uint32_t bhsrIdx = ((uint32_t)( branchPc ^ (branchPc >> NB) ^ (branchPc
    // >> 2*NB) )) & bhsrMask;
    ...
    phtIdx ^=
        (phtIdx & ~phtMask) >>
        (HB -
         LB);  // take the [HB-1, LB] bits of bshr, XOR with [LB-1, ...] bits
    phtIdx &= phtMask;
    ...
    // Update
    pht[phtIdx] =
        taken ? (pred ? 3 : (pht[phtIdx] + 1))
              : (pred ? (pht[phtIdx] - 1) : 0);  // 2-bit saturating counter
    bhsr[bhsrIdx] =
        ((bhsr[bhsrIdx] << 1) & histMask) |
        (taken ? 1 : 0);  // we apply phtMask here, dependence is further away
...
class WindowStructure {
 private:
  // NOTE: Nehalem has POPCNT, but we want this to run reasonably fast on
  // Core2's, so let's keep track of both count and mask.
  struct WinCycle {
    uint8_t occUnits;
    uint8_t count;
    inline void set(uint8_t o, uint8_t c) {
      occUnits = o;
      count = c;
    }
  };
 ...
 public:
  ...
  void schedule(uint64_t& curCycle, uint64_t& schedCycle, uint8_t portMask,
    ...
    if (!extraSlots) {
      ...
      // This is not entirely accurate, as an instruction may have been
      // scheduled already on this port and we'll have a non-contiguous
      // allocation. In practice, this is rare.
      for (uint32_t i = 0; i < extraSlots; i++) {
        scheduleInternal<false, false>(curCycle, extraSlotCycle,
                                       extraSlotPortMask);
    ...
    if (curPos == H) {  // rebase
      ...
      if (!ubWin.empty()) {
        ...
        while (it != ubWin.end() && it->first < nextWinHorizon) {
          ...
          assert_msg(nextWinPos < H,
                     "WindowStructure: ubWin elem exceeds limit cycle=%ld "
                     "curCycle=%ld nextWinPos=%d",
                     it->first, curCycle, nextWinPos);
          nextWin[nextWinPos] = it->second;
          // info("Moved %d events from unbounded window, cycle %ld (%d cycles
          // away)", it->second, it->first, it->first - curCycle);
          it++;
        }
  ...
  void longAdvance(uint64_t& curCycle, uint64_t targetCycle) {
    ...
    if (occupancy) {
      // info("advance: window not drained at %ld, %d uops left", curCycle,
      // occupancy);
      assert(curCycle == targetCycle);
    } else {
      // info("advance: window drained at %ld, jumping to %ld", curCycle,
      // targetCycle);
      assert(curCycle <= targetCycle);
  ...
  void poisonRange(uint64_t curCycle, uint64_t targetCycle, uint8_t portMask) {
    ...
    // info("Poisoned port mask %x from %ld to %ld (tgt %ld)", portMask,
    // curCycle, poisonCycle, targetCycle);
    assert(schedCurCycle == curCycle);
  }

 private:
  template <bool touchOccupancy, bool recordPort>
  void scheduleInternal(uint64_t& curCycle, uint64_t& schedCycle,
                        uint8_t portMask) {
    ...
    while (curWinPos < H) {
      if (trySchedule<touchOccupancy, recordPort>(curWin[curWinPos],
                                                  portMask)) {
    ...
    if (curWinPos >= H) {
      ...
      while (nextWinPos < H) {
        if (trySchedule<touchOccupancy, recordPort>(nextWin[nextWinPos],
                                                    portMask)) {
      ...
      if (nextWinPos >= H) {
        ...
        while (true) {
          if (it == ubWin.end()) {
            ...
            WinCycle wc = {0, 0};
            assert((trySchedule<touchOccupancy, recordPort>(wc, portMask)));
            ubWin.insert(it /*hint, makes insert faster*/,
                         std::pair<uint64_t, WinCycle>(schedCycle, wc));
          } else {
            if (!trySchedule<touchOccupancy, recordPort>(it->second,
                                                         portMask)) {
              // Try next cycle
              it++;
              schedCycle++;
  ...
  inline uint8_t trySchedule(WinCycle& wc, uint8_t portMask) {
    static_assert(!(recordPort && !touchOccupancy),
                  "Can't have recordPort and !touchOccupancy");
    if (touchOccupancy) {
      uint8_t availMask = (~wc.occUnits) & portMask;
      if (availMask) {
        // info("PRE: occUnits=%x portMask=%x availMask=%x", wc.occUnits,
        // portMask, availMask);
        uint8_t firstAvail = __builtin_ffs(availMask) - 1;
        // NOTE: This is not fair across ports. I tried round-robin scheduling,
        // and there is no measurable difference (in our case, fairness comes
        // from following program order)
        if (recordPort) lastPort = firstAvail;
        wc.occUnits |= 1 << firstAvail;
        wc.count++;
        // info("POST: occUnits=%x count=%x firstAvail=%d", wc.occUnits,
        // wc.count, firstAvail);
      }
      return availMask;
    } else {
...
class ReorderBuffer {
 ...
 public:
  ...
  inline uint64_t minAllocCycle() { return buf[idx]; }
...
// Similar to ReorderBuffer, but must have in-order allocations and retires (-->
// faster)
...
class OOOCore : public Core {
 private:
  ...
  uint64_t curCycle;  // this model is issue-centric; curCycle refers to the
                      // current issue cycle
  uint64_t
      regScoreboard[MAX_REGISTERS];  // contains timestamp of next issue cycles
                                     // where each reg can be sourced
  ...
  uint64_t lastStoreAddrCommitCycle;  // tracks last store addr uop, all loads
                                      // queue behind it

  // LSU queues are modeled like the ROB. Surprising? Entries are grabbed in
  // dataflow order, and for ordering purposes should leave in program order. In
  // reality they are associative buffers, but we split the associative
  // component from the limited-size modeling. NOTE: We do not model the
  // 10-entry fill buffer here; the weave model should take care to not overlap
  // more than 10 misses.
  ...
  // This would be something like the Atom... (but careful, the iw probably does
  // not allow 2-wide when configured with 1 slot) WindowStructure<1024, 1
  // /*size*/, 2 /*width*/> insWindow; //this would be something like an Atom,
  // except all the instruction pairing business...

  // Nehalem
  WindowStructure<1024, 36 /*size*/>
      insWindow;  // NOTE: IW width is implicitly determined by the decoder,
                  // which sets the port masks according to uop type
  ReorderBuffer<128, 4> rob;

  // Agner's guide says it's a 2-level pred and BHSR is 18 bits, so this is the
  // config that makes sense; in practice, this is probably closer to the
  // Pentium M's branch predictor, (see Uzelac and Milenkovic, ISPASS 2009),
  // which get the 18 bits of history through a hybrid predictor (2-level +
  // bimodal + loop) where a few of the 2-level history bits are in the tag.
  // Since this is close enough, we'll leave it as is for now. Feel free to
  // reverse-engineer the real thing... UPDATE: Now pht index is XOR-folded
  // BSHR. This has 6656 bytes total -- not negligible, but not ridiculous.
...
#define FWD_ENTRIES 32  // 2 lines, 16 4B entries/line
 ...
 private:
  ...
  // Predicated loads and stores call this function, gets recorded as a 0-cycle
  // op. Predication is rare enough that we don't need to model it perfectly to
  // be accurate (i.e. the uops still execute, retire, etc), but this is needed
  // for correctness.
