...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name,
  ...
  if (!isTerminal) {
    ...
    bool zeroLatencyCache =
        config.get<bool>(prefix + "zeroLatencyCache", false);
...
CacheGroup* BuildCacheGroup(Config& config, const string& name,
  ...
  std::cout << "    building cache: " << prefix << std::endl;
  ...
  if (!prefetch_type.empty()) {  // build a prefetcher group
    if (prefetch_type == "Stride" || prefetch_type == "GHB" ||
      ...
      for (uint32_t i = 0; i < prefetchers; i++) {
        ...
        if (prefetch_type == "GHB") {
          ...
          bool pref_complex = config.get<bool>(prefix + "pref_complex", false);
          bool limit_pref =
              config.get<bool>(prefix + "limit_prefetching", false);
          bool var_degree = config.get<bool>(prefix + "variable_degree", false);
          g_string pref_kernels =
              config.get<const char*>(prefix + "pref_kernels", "");
          cg[i][0] = new DataflowPrefetcher(
              pfName, degree, target_cache, monitor_GETS, monitor_GETX,
              pref_simple, pref_complex, limit_pref, var_degree, NULL,
              pref_kernels);
        } else {
      ...
      g_string pc_vocab_path =
          config.get<const char*>(prefix + "pcVocabPath", "");
      g_string delta_vocab_path =
          config.get<const char*>(prefix + "deltaVocabPath", "");
      g_string model_path = config.get<const char*>(prefix + "modelPath", "");
      double conf_thresh =
          config.get<double>(prefix + "confidence",
                             -0.69);  // high confidence: >50% weight for one
                                      // delta, will emit at most one prefetch
      bool monitor_loads =
          config.get<bool>(prefix + "monitorLoads", true);  // really 'GETS'
      bool monitor_stores =
          config.get<bool>(prefix + "monitorStores", true);  // really 'GETX'
      ...
      for (uint32_t i = 0; i < prefetchers; i++) {
        ...
        ss << name << '-' << i;
        g_string full_name(ss.str().c_str());
        cg[i].emplace_back(new TFPrefetcher(
            full_name, target_cache, monitor_loads, monitor_stores, model_path,
            pc_vocab_path, delta_vocab_path, degree, depth, conf_thresh));
      }
      return cgp;
#endif
...
static void InitSystem(Config& config) {
  ...
  if (!zinfo->traceDriven) {
    ...
    zinfo->readers =
        gm_memalign<TraceReader*>(CACHE_LINE_BYTES, zinfo->numCores);
    coreIdx = 0;
    for (const char* group : coreGroupNames)
    ...
    for (const char* grp : cacheGroupNames) {
      if (isTerminal(grp)) {
        for (vector<BaseCache*> cv : *cMap[grp]) {
          assert(cv.size() == 1);
          TraceDriverProxyCache* proxy = (TraceDriverProxyCache*)(cv[0]);
          assert(proxy);
...
void SimInit(const char* configFile, const char* outputDir, uint32_t shmid) {
  ...
  zinfo->maxMinInstrs = config.get<uint32_t>("sim.maxMinInstrs", 0);
  zinfo->maxTotalInstrs = config.get<uint32_t>("sim.maxTotalInstrs", 0);
