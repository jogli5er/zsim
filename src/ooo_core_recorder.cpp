...
class OOOIssueEvent : public TimingEvent {
 private:
  uint64_t zllStartCycle;  // minStartCycle - gapCycles, stable across
                           // readjustments of gapCycles
  OOOCoreRecorder* cRec;
  uint64_t id;

 public:
  OOOIssueEvent(uint32_t preDelay, uint64_t _zllStartCycle,
                OOOCoreRecorder* _cRec, int32_t domain = -1)
      : TimingEvent(preDelay, 0, domain),
        zllStartCycle(_zllStartCycle),
        cRec(_cRec) {}

  void simulate(uint64_t startCycle) {
    TRACE_MSG("Issue %ld zllStartCycle %ld startCycle %ld minStartCycle %ld",
              id, zllStartCycle, startCycle, getMinStartCycle());
...
class OOODispatchEvent : public TimingEvent {
 ...
 public:
  OOODispatchEvent(uint64_t preDelay, uint64_t _zllStartCycle,
                   int32_t domain = -1)
      : TimingEvent(preDelay, 0, domain), zllStartCycle(_zllStartCycle) {}

  void simulate(uint64_t startCycle) {
    TRACE_MSG("Dispatch %ld zllStartCycle %ld startCycle %ld minStartCycle %ld",
              id, zllStartCycle, startCycle, getMinStartCycle());
    done(startCycle);
  }
...
class OOORespEvent : public TimingEvent {
 ...
 public:
  OOORespEvent(uint64_t preDelay, OOOCoreRecorder* _cRec, int32_t domain = -1)
...
bool OOOCoreRecorder::CompareRespEvents::operator()(
    const FutureResponse& e1, const FutureResponse& e2) const {
...
uint64_t OOOCoreRecorder::notifyJoin(uint64_t curCycle) {
  if (state == HALTED) {
    ...
    lastEvProduced = new (eventRecorder)
        OOOIssueEvent(0, curCycle - gapCycles, this, domain);
    ...
    DEBUG_MSG("[%s] Joined, was HALTED, curCycle %ld halted %ld", name.c_str(),
              curCycle, totalHaltedCycles);
  } else if (state == DRAINING) {
    assert(curCycle >=
           zinfo->globPhaseCycles);  // should not have gone out of sync...
    DEBUG_MSG("[%s] Joined, was DRAINING, curCycle %ld", name.c_str(),
              curCycle);
    assert(lastEvProduced);
...
void OOOCoreRecorder::addIssueEvent(uint64_t evCycle) {
  ...
  assert_msg(zllCycle >= lastEvProduced->zllStartCycle, "zllCycle %ld last %ld",
             zllCycle, lastEvProduced->zllStartCycle);
  OOOIssueEvent* ev =
      new (eventRecorder) OOOIssueEvent(0, zllCycle, this, domain);
  ev->id = curId++;
  ...
  while (!futureResponses.empty()) {
    ...
    if (firstResp.ev) {
      TRACE_MSG("linked Issue zll %ld with Resp zll %ld", zllCycle,
                firstResp.zllStartCycle);
  ...
  uint32_t preDelay =
      maxCycle ? ((maxCycle < zllCycle) ? (zllCycle - maxCycle) : 0) : 0;
  ...
  TRACE_MSG("linked Issue zll %ld with prev Issue, delay %d", zllCycle,
            issueDelay);
...
S& GetPrioQueueContainer(std::priority_queue<T, S, C>& q) {
  struct PQE : private std::priority_queue<T, S, C> {
    static S& Container(std::priority_queue<T, S, C>& q) { return q.*&PQE::c; }
  };
  return PQE::Container(q);
}

void OOOCoreRecorder::recordAccess(uint64_t curCycle, uint64_t dispatchCycle,
                                   uint64_t respCycle) {
  ...
  if (IsGet(tr.type)) {
    assert(tr.endEvent);
    // info("Handling GET: curCycle %ld ev(reqCycle %ld respCycle %ld) respCycle
    // %ld", curCycle, tr.reqCycle, tr.respCycle, respCycle);

    addIssueEvent(curCycle);

    // Delay
    DelayEvent* dDisp =
        new (eventRecorder) DelayEvent(dispatchCycle - curCycle);
    dDisp->setMinStartCycle(curCycle);

    // Dispatch event
    OOODispatchEvent* dispEv = new (eventRecorder)
        OOODispatchEvent(/*dispatchCycle - curCycle*/ 0, dispatchCycle);
    ...
    for (FutureResponse& fr : GetPrioQueueContainer(futureResponses)) {
      if (fr.zllStartCycle < zllDispatchCycle && fr.ev) {
        DelayEvent* dl =
            new (eventRecorder) DelayEvent(zllDispatchCycle - fr.zllStartCycle);
        fr.ev->addChild(dl, eventRecorder)->addChild(dispEv, eventRecorder);
      }
    }
    // Link request
    DelayEvent* dUp = new (eventRecorder) DelayEvent(
        tr.reqCycle - dispatchCycle);  // TODO: remove, postdelay in dispatch...
    dUp->setMinStartCycle(dispatchCycle);
    lastEvProduced->addChild(dDisp, eventRecorder)
        ->addChild(dispEv, eventRecorder)
        ->addChild(dUp, eventRecorder)
    ...
    OOORespEvent* respEvent =
        new (eventRecorder) OOORespEvent(downDelay, this, domain);
    respEvent->id = curId++;
    respEvent->setMinStartCycle(respCycle);
    tr.endEvent->addChild(respEvent, eventRecorder);
    TRACE_MSG("Adding resp zllCycle %ld delay %ld", respCycle - gapCycles,
              respCycle - curCycle);
    ...
    lastEvProduced->addChild(putUp, eventRecorder)
        ->addChild(tr.startEvent, eventRecorder);

    // PUT's endEvent not linked to anything, it's a wback in some cache above
    // and we should not capture it
  }
...
uint64_t OOOCoreRecorder::cSimStart(uint64_t curCycle) {
  ...
  } else if (state == DRAINING) {  // add no event --- that's how we detect
                                   // we're done draining
    // Drain futureResponses... we could be a bit more exact by doing partial
    // drains, but if the thread has not joined back by the end of phase,
    // chances are this is a long leave
...
uint64_t OOOCoreRecorder::cSimEnd(uint64_t curCycle) {
  ...
  // In our current clock, when did the last event start (1) before contention
  // simulation, and (2) after contention simulation
  uint64_t lastEvCycle1 =
      lastEvSimulatedZllStartCycle +
      gapCycles;  // we add gapCycles because zllStartCycle is in zll clocks
  uint64_t lastEvCycle2 = lastEvSimulatedStartCycle;

  assert(lastEvCycle1 <= curCycle);
  assert_msg(lastEvCycle2 <= curCycle, "[%s] lec2 %ld cc %ld, state %d",
             name.c_str(), lastEvCycle2, curCycle, state);
  if (unlikely(lastEvCycle1 > lastEvCycle2))
    panic(
        "[%s] Contention simulation introduced a negative skew, curCycle %ld, "
        "lc1 %ld lc2 %ld, gapCycles %ld",
        name.c_str(), curCycle, lastEvCycle1, lastEvCycle2, gapCycles);

  uint64_t skew = lastEvCycle2 - lastEvCycle1;

  // Skew clock
  // Note that by adding to gapCycles, we keep the zll clock (defined as
  // curCycle - gapCycles) constant. We use the zll clock to translate
  // origStartCycle correctly, even if it's coming from several phases back.
  curCycle += skew;
  gapCycles += skew;
  eventRecorder.setGapCycles(gapCycles);
  // We deal with all our events in zllCycles, so no need to update any event
  // counts

  // NOTE: Suppose that we had a really long event, so long that in the next
  // phase, lastEvSimulated is still the same. In this case, skew will be 0, so
  // we do not need to remove it.

  DEBUG_MSG("[%s] curCycle %ld zllCurCycle %ld lec1 %ld lec2 %ld skew %ld",
            name.c_str(), curCycle, curCycle - gapCycles, lastEvCycle1,
            lastEvCycle2, skew);
  ...
  if (!lastEvProduced) {
    ...
    assert_msg(state == DRAINING,
               "[%s] state %d lastEvSimulated startCycle %ld curCycle %ld",
               name.c_str(), state, lastEvSimulatedStartCycle, curCycle);
    lastUnhaltedCycle =
        lastEvSimulatedStartCycle;  // the taper is a 0-delay event
    state = HALTED;
    DEBUG_MSG(
        "[%s] lastEvSimulated reached (startCycle %ld), DRAINING -> HALTED",
        name.c_str(), lastEvSimulatedStartCycle);
    assert(futureResponses.empty());
    // This works (because we flush on leave()) but would be inaccurate if we
    // called leave() very frequently; now leave() only happens on blocking
    // syscalls though
  }
  return curCycle;
}
...
uint64_t OOOCoreRecorder::getUnhaltedCycles(uint64_t curCycle) const {
  ...
  uint64_t haltedCycles =
      totalHaltedCycles + ((state == HALTED) ? (cycle - lastUnhaltedCycle) : 0);
