...
TimingCore::TimingCore(FilterCache* _l1i, FilterCache* _l1d, uint32_t _domain,
                       g_string& _name)
    : Core(_name),
      l1i(_l1i),
      l1d(_l1d),
...
void TimingCore::bblAndRecord(Address bblAddr, BblInfo* bblInfo) {
  ...
  for (Address fetchAddr = bblAddr; fetchAddr < endBblAddr;
    ...
    uint64_t startCycle = curCycle;
    curCycle = l1i->load(fetchAddr, curCycle, 0 /*no PC*/, (1 << lineBits));
    cRec.record(startCycle);
  }
}

InstrFuncPtrs TimingCore::GetFuncPtrs() {
  return {LoadAndRecordFunc,     StoreAndRecordFunc,
          BblAndRecordFunc,      BranchFunc,
          PredLoadAndRecordFunc, PredStoreAndRecordFunc,
          FPTR_ANALYSIS,         {0}};
}
...
void TimingCore::BblAndRecordFunc(THREADID tid, ADDRINT bblAddr,
                                  BblInfo* bblInfo) {
...
void TimingCore::PredLoadAndRecordFunc(THREADID tid, ADDRINT addr, ADDRINT pc,
                                       BOOL pred) {
  if (pred) static_cast<TimingCore*>(cores[tid])->loadAndRecord(addr, pc);
}

void TimingCore::PredStoreAndRecordFunc(THREADID tid, ADDRINT addr, ADDRINT pc,
                                        BOOL pred) {
