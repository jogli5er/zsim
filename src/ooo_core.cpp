 ...
 * up. In theory, more detailed, but sometimes much slower (as it relies on
 * range poisoning in the IW, potentially O(n^2)), and in practice makes a
 * negligible difference (ROB backpressures).
 */
...
// Stages --- more or less matched to Westmere, but have not seen detailed pipe
// diagrams anywhare
...
OOOCore::OOOCore(OOOFilterCache* _l1i, OOOFilterCache* _l1d, g_string& _name,
  ...
  branchPred = new BranchPredictorPAg(properties->bp_nb, properties->bp_hb,
                                      properties->bp_lb);
}
...
void OOOCore::initStats(AggregateStat* parentStat) {
  ...
  approxInstrsStat->init("approxInstrs", "Instrs with approx uop decoding",
                         &approxInstrs);
  ...
  coreStat->append(&disp);
  coreStat->append(&lowBits);
  profCondBranches.init("condBranches", "Conditional Branches");
  coreStat->append(&profCondBranches);
  ...
  profRFReadStalls.init("rfStalls", "Register File read stalls");
  coreStat->append(&profRFReadStalls);
  profRobStalls.init("robStalls", "Reorder Buffer stalls");
  coreStat->append(&profRobStalls);
...
uint64_t OOOCore::getPhaseCycles() const {
...
InstrFuncPtrs OOOCore::GetFuncPtrs() {
  return {LoadFunc,     StoreFunc,     BblFunc,       BranchFunc,
          PredLoadFunc, PredStoreFunc, FPTR_ANALYSIS, {0}};
}

inline void OOOCore::load(Address addr) { loadAddrs[loads++] = addr; }

void OOOCore::store(Address addr) { storeAddrs[stores++] = addr; }

// Predicated loads and stores call this function, gets recorded as a 0-cycle
// op. Predication is rare enough that we don't need to model it perfectly to be
// accurate (i.e. the uops still execute, retire, etc), but this is needed for
// correctness.
void OOOCore::predFalseMemOp() {
  // I'm going to go out on a limb and assume just loads are predicated (this
  // will not fail silently if it's a store)
  loadAddrs[loads++] = -1L;
}
...
inline void OOOCore::bbl(Address bblAddr, BblInfo* bblInfo) {
  ...
  for (uint32_t i = 0; i < bbl->uops; i++) {
    ...
    if (decodeCycle > curCycle) {
      // info("Decode stall %ld %ld | %d %d", decodeCycle, curCycle,
      // uop->decCycle, prevDecCycle);
      uint32_t cdDiff = decodeCycle - curCycle;
#ifdef OOO_STALL_STATS
      // Only account for actual decode stalls, as fetch also modifies
      // decodeCycle
    ...
    // Using curCycle saves us two unpredictable branches in the RF read stalls
    // code
    ...
    uint64_t dispatchCycle =
        MAX(cOps, MAX(c2, c3) + (DISPATCH_STAGE - ISSUE_STAGE));

    // info("IW 0x%lx %d %ld %ld %x", bblAddr, i, c2, dispatchCycle,
    // uop->portMask); NOTE: Schedule can adjust both cur and dispatch cycles
    ...
    switch (uop->type) {
      ...
      case UopType::LOAD: {
        ...
        if (addr != ((Address)-1L)) {
          if (uop->type == UopType::SW_DATA_PREFETCH) {
            ...
            reqSatisfiedCycle = l1d->issuePrefetch(
        ...
        if (fwdArray[fwdIdx].addr == addr) {
          // info("0x%lx FWD %ld %ld", addr, reqSatisfiedCycle,
          // fwdArray[fwdIdx].storeCycle);
           ...
           */
          reqSatisfiedCycle =
              MAX(reqSatisfiedCycle, fwdArray[fwdIdx].storeCycle);
        }
      ...
      case UopType::FENCE:
        commitCycle = dispatchCycle + uop->lat;
        // info("%d %ld %ld", uop->lat, lastStoreAddrCommitCycle,
        // lastStoreCommitCycle); force future load serialization
        lastStoreAddrCommitCycle =
            MAX(commitCycle,
                MAX(lastStoreAddrCommitCycle, lastStoreCommitCycle + uop->lat));
        // info("%d %ld %ld X", uop->lat, lastStoreAddrCommitCycle,
        // lastStoreCommitCycle);
        break;
    }
    ...
    // info("0x%lx %3d [%3d %3d] -> [%3d %3d]  %8ld %8ld %8ld %8ld", bbl->addr,
    // i, uop->rs[0], uop->rs[1], uop->rd[0], uop->rd[1], decCycle, c3,
    // dispatchCycle, commitCycle);
  }
  ...
  assert_msg(loadIdx == loads, "%s: loadIdx(%d) != loads (%d)", name.c_str(),
             loadIdx, loads);
  assert_msg(storeIdx == stores, "%s: storeIdx(%d) != stores (%d)",
             name.c_str(), storeIdx, stores);
  loads = stores = 0;
  ...
  // This method is the consumer of BblInfo objects, and as such it has the
  // responsibility to free memory resources if the BBL is not cached.
  // (Otherwise the caller or creater of BblInfo objects would have to somehow
  // track when we were done using them.)
  if (!prevBbl->preserve) {
    gm_free(prevBbl);
  }
  prevBbl = bblInfo;
...
void OOOCore::join() {
  DEBUG_MSG("[%s] Joining, curCycle %ld phaseEnd %ld", name.c_str(), curCycle,
            phaseEndCycle);
  ...
  DEBUG_MSG("[%s] Joined, curCycle %ld phaseEnd %ld", name.c_str(), curCycle,
            phaseEndCycle);
}

void OOOCore::leave() {
  DEBUG_MSG("[%s] Leaving, curCycle %ld phaseEnd %ld", name.c_str(), curCycle,
            phaseEndCycle);
...
void OOOCore::LoadFunc(THREADID tid, ADDRINT addr, ADDRINT) {
  static_cast<OOOCore*>(cores[tid])->load(addr);
}
void OOOCore::StoreFunc(THREADID tid, ADDRINT addr, ADDRINT) {
...
void OOOCore::BblFunc(THREADID tid, ADDRINT bblAddr, BblInfo* bblInfo) {
  ...
  while (core->curCycle > core->phaseEndCycle) {
    ...
    // NOTE: TakeBarrier may take ownership of the core, and so it will be used
    // by some other thread. If TakeBarrier context-switches us, the *only* safe
    // option is to return inmmediately after we detect this, or we can race and
    // corrupt core state. However, the information here is insufficient to do
    // that, so we could wind up double-counting phases.
    uint32_t newCid = TakeBarrier(tid, cid);
    // NOTE: Upon further observation, we cannot race if newCid == cid, so this
    // code should be enough. It may happen that we had an intervening
    // context-switch and we are now back to the same core. This is fine, since
    // the loop looks at core values directly and there are no locals involved,
