...
Cache::Cache(uint32_t _numLines, CC* _cc, CacheArray* _array, ReplPolicy* _rp,
...
const char* Cache::getName() { return name.c_str(); }

void Cache::setParents(uint32_t childId, const g_vector<MemObject*>& parents,
                       Network* network) {
  cc->setParents(childId, parents, network);
}

void Cache::setChildren(const g_vector<BaseCache*>& children,
                        Network* network) {
...
uint64_t Cache::access(MemReq& req) {
  ...
  if (likely(!skipAccess)) {
    ...
    if (lineId != -1 && cc->isValid(lineId)) {
      ...
      // This also fixes a (relatively infrequent) timing bug in the filter
      // cache where the available cycle info is lost if a block in a set is
      // replaced and then used again before the cycle is reached. Add accLat if
      // a) line is in the cache or b) line is not in the cache. If c) line is
      // in-flight (prefetch) serve from MSHR and do not account for accLat
    ...
    if (lineId == -1 && cc->shouldAllocate(req)) {
      ...
      // do not include their delays NOTE: We might be "evicting" an invalid
      // line for all we know. Coherence controllers will know what to do
      cc->processEviction(req, wbLineAddr, lineId,
                          respCycle);  // 1. if needed, send
                                       // invalidates/downgrades to lower level

      need_postinsert = true;  // defer the actual insertion until we know its
                               // cycle availability
    }
#ifndef EXTERNAL_CACHE_MODEL
    ...
    if (need_postinsert) {
      array->postinsert(
          req.lineAddr, &req, lineId,
          respCycle);  // do the actual insertion. NOTE: Now we must split
                       // insert into a 2-phase thing because cc unlocks us.
    }
...
void Cache::startInvalidate() {
  cc->startInv();  // note we don't grab tcc; tcc serializes multiple up
...
uint64_t Cache::finishInvalidate(const InvReq& req) {
  ...
  assert_msg(lineId != -1,
             "[%s] Invalidate on non-existing address 0x%lx type %s lineId %d, "
             "reqWriteback %d",
             name.c_str(), req.lineAddr, InvTypeName(req.type), lineId,
             *req.writeback);
  uint64_t respCycle = req.cycle + invLat;
  ZSIM_TRACE(Cache,
             "[%s] Invalidate start 0x%lx type %s lineId %d, reqWriteback %d",
             name.c_str(), req.lineAddr, InvTypeName(req.type), lineId,
             *req.writeback);
  respCycle = cc->processInv(req, lineId,
                             respCycle);  // send invalidates or downgrades to
                                          // children, and adjust our own state
  ZSIM_TRACE(Cache,
             "[%s] Invalidate end 0x%lx type %s lineId %d, reqWriteback %d, "
             "latency %ld",
             name.c_str(), req.lineAddr, InvTypeName(req.type), lineId,
             *req.writeback, respCycle - req.cycle);
