...
// TODO: Now that we have a pure CC interface, the MESI controllers should go on
// different files.
...
class CC : public GlobAlloc {
 public:
  // Initialization
  virtual void setParents(uint32_t childId, const g_vector<MemObject*>& parents,
                          Network* network) = 0;
  virtual void setChildren(const g_vector<BaseCache*>& children,
                           Network* network) = 0;
  ...
  virtual bool startAccess(
      MemReq& req) = 0;  // initial locking, address races; returns true if
                         // access should be skipped; may change req!
  virtual bool shouldAllocate(
      const MemReq&
          req) = 0;  // called when we don't find req's lineAddr in the array
  virtual uint64_t processEviction(
      const MemReq& triggerReq, Address wbLineAddr, int32_t lineId,
      uint64_t startCycle) = 0;  // called iff shouldAllocate returns true
  virtual uint64_t processAccess(const MemReq& req, int32_t lineId,
                                 uint64_t startCycle,
                                 uint64_t* getDoneCycle = nullptr) = 0;
  virtual void endAccess(const MemReq& req) = 0;

  // Inv methods
  virtual void startInv() = 0;
  virtual uint64_t processInv(const InvReq& req, int32_t lineId,
 ...
 *  - The BOTTOM controller, which deals with keeping coherence state with
 * respect to the upper level and issues requests (accesses) to upper levels.
 *  - The TOP controller, which keeps state of lines w.r.t. lower levels of the
 * hierarchy (e.g. sharer lists), and issues requests (invalidates) to lower
 * levels. The naming scheme is PROTOCOL-CENTRIC, i.e. if you draw a multi-level
 * hierarchy, between each pair of levels there is a top CC at the top and a
 * bottom CC at the bottom. Unfortunately, if you look at the caches, the bottom
 * CC is at the top and the top CC is at the bottom. So the cache class may seem
 * a bit weird at times, but the controller classes make more sense. Here's an
 * example:
 *
...
/* NOTE: To avoid virtual function overheads, there is no BottomCC interface,
 * since we only have a MESI controller for now */
...
class MESIBottomCC : public GlobAlloc {
 ...
 public:
  MESIBottomCC(uint32_t _numLines, uint32_t _selfId, bool _nonInclusiveHack)
      : numLines(_numLines),
        selfId(_selfId),
  ...
  void initStats(AggregateStat* parentStat) {
    ...
    profSpecGETSHit.init("hSpecGETS",
                         "Speculative GETS hits (disjoint from demand)");
    profSpecGETSMiss.init("mSpecGETS",
                          "Speculative GETS misses (disjoint from demand)");
    profSWSpecGETSHit.init(
        "hSWSpecGETS",
        "SW prefetch speculative GETS hits (disjoint from demand)");
    profSWSpecGETSMiss.init(
        "mSWSpecGETS",
        "SW prefetch speculative GETS misses (disjoint from demand)");
  ...
  uint64_t processAccess(MemReq req, int32_t lineId, uint64_t cycle,
                         uint32_t flags);
  ...
  g_vector<MemObject*>* getParents() { return &parents; }

 private:
  uint32_t getParentId(Address lineAddr);
};

// Implements the "top" part: Keeps directory information, handles downgrades
// and invalidates
class MESITopCC : public GlobAlloc {
 ...
 public:
  ...
  g_vector<BaseCache*>* getChildren() { return &children; }
...
static inline bool CheckForMESIRace(AccessType& type, MESIState* state,
  ...
  if (*state != initialState) {
    ...
    // mesiStateNames[initialState], mesiStateNames[*state]); An intervening
    // invalidate happened! Two types of races:
...
class MESICC : public CC {
 ...
 public:
  ...
  g_vector<MemObject*>* getParents() override { return bcc->getParents(); }

  g_vector<BaseCache*>* getChildren() override { return tcc->getChildren(); }
  ...
  bool startAccess(MemReq& req) {
     ...
     * going down (which is why we require the lock), but not when going up,
     * opening the child to invalidation races here to avoid deadlocks.
     */
    if (req.childLock) {
     ...
     * state, because we hold both parent's locks. So, we first handle races,
     * which may cause us to skip the access.
     */
    bool skipAccess =
        CheckForMESIRace(req.type /*may change*/, req.state, req.initialState);
    return skipAccess;
  }
  ...
  bool shouldAllocate(const MemReq& req) {
    if ((req.type == GETS) || (req.type == GETX)) {
      ...
      if (!nonInclusiveHack) {
        panic(
            "[%s] We lost inclusion on this line! 0x%lx, type %s, childId %d, "
            "childState %s",
            name.c_str(), req.lineAddr, AccessTypeName(req.type), req.childId,
            MESIStateName(*req.state));
      }
      return false;
    }
  }
  ...
  uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle,
    ...
    // may start becoming more common (and it is perfectly acceptable to upgrade
    // without any interaction with the parent... the child had the
    // permissions!)
    if (!req.prefetch &&
        (lineId == -1 ||
         (((req.type == PUTS) || (req.type == PUTX)) &&
          !bcc->isValid(lineId)))) {  // can only be a non-inclusive wback
      assert(nonInclusiveHack);
      assert((req.type == PUTS) || (req.type == PUTX));
      respCycle = bcc->processNonInclusiveWriteback(
          req.lineAddr, req.type, startCycle, req.state, req.srcId, req.flags);
    } else {
      ...
      // The PREFETCH flag must be cleared in the cache which receives the
      // speculative request. It cannot propagate up after this since this flag
      // effectively says "Don't assign this line to any children" by preventing
      // TCC from updating its directory.
      ...
      respCycle = bcc->processAccess(req, lineId, startCycle, flags);
      ...
      if (!isPrefetch) {  // prefetches only touch bcc; the demand request from
        ...
        respCycle = tcc->processAccess(req.lineAddr, lineId, req.type,
                                       req.childId, bcc->isExclusive(lineId),
                                       req.state, &lowerLevelWriteback,
                                       respCycle, req.srcId, flags);
...
class MESITerminalCC : public CC {
 ...
 public:
  ...
  bool shouldAllocate(const MemReq& req) { return (req.prefetch == 0); }
  ...
  uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle,
    ...
    // If this is a prefetch that doesn't skip any cache levels, clear the
    // PREFETCH flag so that normal TCC/directory processing occurs
    uint32_t flags = req.flags;
    if (req.prefetch == 0) {
      flags &= ~MemReq::PREFETCH;
    }
    ...
    // if needed, fetch line or upgrade miss from upper level
    uint64_t respCycle = bcc->processAccess(req, lineId, startCycle, flags);
    // at this point, the line is in a good state w.r.t. upper levels
    return respCycle;
  }
