...
#if defined(LIBCONFIGXX_VER_MAJOR) && defined(LIBCONFIGXX_VER_MINOR) && \
    defined(LIBCONFIGXX_VER_REVISION)
#define LIBCONFIG_VERSION                                        \
  (LIBCONFIGXX_VER_MAJOR * 10000 + LIBCONFIGXX_VER_MINOR * 100 + \
...
Config::Config(const char* inFile) {
  ...
  try {
  ...
  } catch (libconfig::FileIOException const& fioe) {
    panic("Input config file %s could not be read", inFile);
  } catch (libconfig::ParseException const& pe) {
    ...
    // Using inFile is typically OK, but won't be accurate with multi-file
    // configs (includes)
    const char* peFile = inFile;
#endif
    panic("Input config file %s could not be parsed, line %d, error: %s",
          peFile, pe.getLine(), pe.getError());
  }
}
...
// Helper function: Add "*"-prefixed vars, which are used by our scripts but not
// zsim, to outCfg Returns number of copied vars
static uint32_t copyNonSimVars(libconfig::Setting& s1, libconfig::Setting& s2,
                               std::string prefix) {
  uint32_t copied = 0;
  for (uint32_t i = 0; i < (uint32_t)s1.getLength(); i++) {
    ...
    if (name[0] == '*') {
      if (s2.exists(name))
        panic("Setting %s was read, should be private",
              (prefix + name).c_str());
      ...
      if (libconfig::Setting::Type::TypeInt == s1[i].getType())
        ...
        panic("Unknown type for priv setting %s, cannot copy",
              (prefix + name).c_str());
      copied++;
    }
...
static uint32_t checkIncluded(libconfig::Setting& s1, libconfig::Setting& s2,
                              std::string prefix) {
  uint32_t unused = 0;
  for (uint32_t i = 0; i < (uint32_t)s1.getLength(); i++) {
    const char* name = s1[i].getName();
...
void Config::writeAndClose(const char* outFile, bool strictCheck) {
  uint32_t nonSimVars =
      copyNonSimVars(inCfg->getRoot(), outCfg->getRoot(), std::string(""));
  uint32_t unused =
      checkIncluded(inCfg->getRoot(), outCfg->getRoot(), std::string(""));

  if (nonSimVars)
    info("Copied %d non-sim var%s to output config", nonSimVars,
         (nonSimVars > 1) ? "s" : "");
  if (unused) {
    if (strictCheck) {
      panic("%d setting%s not used during configuration", unused,
            (unused > 1) ? "s" : "");
    } else {
      warn("%d setting%s not used during configuration", unused,
           (unused > 1) ? "s" : "");
    }
  }
...
bool Config::exists(const char* key) { return inCfg->exists(key); }
...
template <>
const char* getTypeName<lc_int64>() {
  return "uint64";
}
template <>
const char* getTypeName<bool>() {
  return "bool";
}
template <>
const char* getTypeName<const char*>() {
  return "string";
}
template <>
...
template <>
SType getSType<const char*>() {
  return SType::TypeString;
}
template <>
SType getSType<double>() {
...
template <>
bool getEq<int>(int v1, int v2) {
  return v1 == v2;
}
template <>
bool getEq<lc_int64>(lc_int64 v1, lc_int64 v2) {
  return v1 == v2;
}
template <>
bool getEq<bool>(bool v1, bool v2) {
  return v1 == v2;
}
template <>
bool getEq<const char*>(const char* v1, const char* v2) {
  return strcmp(v1, v2) == 0;
}
template <>
bool getEq<double>(double v1, double v2) {
  return v1 == v2;
}
...
static void writeVar(libconfig::Setting& setting, const char* key, T val) {
  ...
  if (sep) {
    ...
    if (!setting.exists(prefix)) {
      ...
      } catch (libconfig::SettingNameException const& sne) {
    ...
    if (!setting.exists(key)) {
      ...
      // If this panics, what the hell are you doing in the code? Multiple reads
      // and different defaults??
      T origVal = setting[key];
      if (!getEq(val, origVal))
        panic("Duplicate writes to out config key %s with different values!",
              key);
    }
  }
}
...
T Config::genericGet(const char* key, T def) {
  ...
  if (inCfg->exists(key)) {
    if (!inCfg->lookupValue(key, val)) {
      panic("Type error on optional setting %s, expected type %s", key,
            getTypeName<T>());
    }
  } else {
    val = def;
...
T Config::genericGet(const char* key) {
  ...
  if (inCfg->exists(key)) {
    if (!inCfg->lookupValue(key, val)) {
      panic("Type error on mandatory setting %s, expected type %s", key,
            getTypeName<T>());
    }
  } else {
...
template <>
const char* Config::get<const char*>(const char* key) {
...
template <>
uint32_t Config::get<uint32_t>(const char* key, uint32_t def) {
...
template <>
bool Config::get<bool>(const char* key, bool def) {
  return genericGet<bool>(key, def);
}
template <>
const char* Config::get<const char*>(const char* key, const char* def) {
  return genericGet<const char*>(key, def);
}
template <>
double Config::get<double>(const char* key, double def) {
...
void Tokenize(const string& str, vector<string>& tokens,
  ...
  string::size_type lastPos =
      0;  // dsm: DON'T //str.find_first_not_of(delimiters, 0);
  ...
  void fill(vector<bool>& mask) {
    for (int32_t i = min; i < sup; i += step) {
      if (i >= (int32_t)mask.size() || i < 0)
        panic("Range %d:%d:%d includes out-of-bounds %d (mask limit %ld)", min,
              step, sup, i, mask.size() - 1);
      mask[i] = true;
    }
  }
};
