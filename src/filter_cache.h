 ...
 * holds the most recently used line in each set. Accesses check the filter
 * array, and then go through the normal access path. Because there is one line
 * per set, it is fine to do this without grabbing a lock.
 */
...
class FilterCache : public Cache {
 ...
 public:
  FilterCache(uint32_t _numSets, uint32_t _numLines, CC* _cc,
  ...
  uint64_t replace(Address vAddr, uint32_t idx, bool isLoad, uint64_t curCycle,
    ...
    // invalidated, but we have the new address guaranteed until we release the
    // lock
  ...
  void schedulePrefetch(Address lineAddr, uint32_t skip, uint64_t pc, bool isSW,
                        bool serialize) {
    // A prefetch from any level of the memory hierarchy is inserted in the
    // filter cache queue of the core whose demand access triggered the
    // prefetch. By originating all prefetch requests from the filter caches, we
    // avoid races and complexities that come with injecting new accesses into
    // an in-progress request. The 'skip' parameter is decremented at each cache
    // level and the prefetch block is allocated starting at the cache for which
    // the value is zero.
  ...
  void executePrefetch(uint64_t curCycle, uint64_t dispatchCycle,
                       uint64_t reqSatisfiedCycle, OOOCoreRecorder* cRec) {
    ...
    if (unlikely(!prefetchQueue.empty())) {
      ...
      for (auto& prefetch : prefetchQueue) {
        ...
        MemReq req = {prefetch.pc,
                      ...
                      dummyState,
                      srcId,
                      MemReq::PREFETCH | MemReq::SPECULATIVE |
                          (prefetch.isSW ? MemReq::SW_SPECULATIVE : 0U),
                      prefetch.skip};
