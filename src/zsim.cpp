...
/* Command-line switches (used to pass info from harness that cannot be passed
 * through the config file, most config is file-based) */

KNOB<INT32> KnobProcIdx(KNOB_MODE_WRITEONCE, "pintool", "procIdx", "0",
                        "zsim process idx (internal)");

KNOB<INT32> KnobShmid(
    KNOB_MODE_WRITEONCE, "pintool", "shmid", "0",
    "SysV IPC shared memory id used when running in multi-process mode");

KNOB<string> KnobConfigFile(
    KNOB_MODE_WRITEONCE, "pintool", "config", "zsim.cfg",
    "config file name (only needed for the first simulated process)");

// We need to know these as soon as we start, otherwise we could not log
// anything until we attach and read the config
KNOB<bool> KnobLogToFile(
    KNOB_MODE_WRITEONCE, "pintool", "logToFile", "false",
    "true if all messages should be logged to a logfile instead of stdout/err");

KNOB<string> KnobOutputDir(KNOB_MODE_WRITEONCE, "pintool", "outputDir", "./",
                           "absolute path to write output files into");
...
uint32_t getCid(uint32_t tid) {
  // assert(tid < MAX_THREADS); //these assertions are fine, but getCid is
  // called everywhere, so they are expensive!
...
VOID PIN_FAST_ANALYSIS_CALL IndirectBasicBlock(THREADID tid, ADDRINT bblAddr,
                                               BblInfo* bblInfo) {
  fPtrs[tid].bblPtr(tid, bblAddr, bblInfo);
}

VOID PIN_FAST_ANALYSIS_CALL IndirectRecordBranch(THREADID tid, ADDRINT branchPc,
                                                 BOOL taken, ADDRINT takenNpc,
                                                 ADDRINT notTakenNpc) {
  fPtrs[tid].branchPtr(tid, branchPc, taken, takenNpc, notTakenNpc);
}

VOID PIN_FAST_ANALYSIS_CALL IndirectPredLoadSingle(THREADID tid, ADDRINT addr,
                                                   ADDRINT pc, BOOL pred) {
  fPtrs[tid].predLoadPtr(tid, addr, pc, pred);
}

VOID PIN_FAST_ANALYSIS_CALL IndirectPredStoreSingle(THREADID tid, ADDRINT addr,
                                                    ADDRINT pc, BOOL pred) {
...
// Join variants: Call join on the next instrumentation poin and return to
// analysis code
...
VOID JoinAndRecordBranch(THREADID tid, ADDRINT branchPc, BOOL taken,
                         ADDRINT takenNpc, ADDRINT notTakenNpc) {
  Join(tid);
  fPtrs[tid].branchPtr(tid, branchPc, taken, takenNpc, notTakenNpc);
}
...
VOID NOPRecordBranch(THREADID tid, ADDRINT addr, BOOL taken, ADDRINT takenNpc,
                     ADDRINT notTakenNpc) {}
...
// TODO (dsm): Went for quick, dirty and contained here. This could use a
// cleanup.
...
// Track the non-FF instructions executed at the beginning of this and last
// interval. Can only be updated at ends of phase, by the NFF tracking event.
static uint64_t* ffiFFStartInstrs;  // hack, needs to be a pointer, written to
...
VOID FFITrackNFFInterval() {
  ...
  assert(
      ffiInstrsDone <
      ffiInstrsLimit);  // unless you have ~10-instr FFWds, this does not happen
  ...
  auto ffiGet = [p, startInstrs]() {
    return zinfo->processStats->getProcessInstrs(p) - startInstrs;
  };
  auto ffiFire = [p, _ffiFFStartInstrs, _ffiPrevFFStartInstrs]() {
    info("FFI: Entering fast-forward for process %d", p);
    /* Note this is sufficient due to the lack of reinstruments on FF, and this
     * way we do not need to touch global state */
  ...
  zinfo->eventQueue->insert(makeAdaptiveEvent(ffiGet, ffiFire, 0,
                                              ffiInstrsLimit - ffiInstrsDone,
...
VOID FFIAdvance() {
  ...
  if (ffiPoint >= ffiPoints.size()) {
    info("Last ffiPoint reached, %ld instrs, limit %ld", ffiInstrsDone,
         ffiInstrsLimit);
    SimEnd();
  } else {
    info("ffiPoint reached, %ld instrs, limit %ld", ffiInstrsDone,
         ffiInstrsLimit);
...
VOID FFIEntryBasicBlock(THREADID tid, ADDRINT bblAddr, BblInfo* bblInfo) {
  ffiInstrsDone +=
      *ffiFFStartInstrs -
      *ffiPrevFFStartInstrs;  // add all instructions executed in the NFF phase
  FFIAdvance();
  assert(ffiNFF);
  ffiNFF = false;
  fPtrs[tid] = GetFFPtrs();
...
static const InstrFuncPtrs joinPtrs = {JoinAndLoadSingle,
                                       JoinAndStoreSingle,
                                       JoinAndBasicBlock,
                                       JoinAndRecordBranch,
                                       JoinAndPredLoadSingle,
                                       JoinAndPredStoreSingle,
                                       FPTR_JOIN};
static const InstrFuncPtrs nopPtrs = {NOPLoadStoreSingle,
                                      NOPLoadStoreSingle,
                                      NOPBasicBlock,
                                      NOPRecordBranch,
                                      NOPPredLoadStoreSingle,
                                      NOPPredLoadStoreSingle,
                                      FPTR_NOP};
static const InstrFuncPtrs retryPtrs = {
    NOPLoadStoreSingle, NOPLoadStoreSingle,     NOPBasicBlock,
    NOPRecordBranch,    NOPPredLoadStoreSingle, NOPPredLoadStoreSingle,
    FPTR_RETRY};
static const InstrFuncPtrs ffPtrs = {NOPLoadStoreSingle,
                                     NOPLoadStoreSingle,
                                     FFBasicBlock,
                                     NOPRecordBranch,
                                     NOPPredLoadStoreSingle,
...
static const InstrFuncPtrs ffiPtrs = {NOPLoadStoreSingle,
                                      NOPLoadStoreSingle,
                                      FFIBasicBlock,
                                      NOPRecordBranch,
                                      NOPPredLoadStoreSingle,
                                      NOPPredLoadStoreSingle,
                                      FPTR_NOP};
static const InstrFuncPtrs ffiEntryPtrs = {NOPLoadStoreSingle,
                                           NOPLoadStoreSingle,
                                           FFIEntryBasicBlock,
                                           NOPRecordBranch,
                                           NOPPredLoadStoreSingle,
...
void EnterFastForward() {
  ...
  // Transition to FF; we have the ff lock, this should be safe with end of
  // phase code. This avoids profiling the end of a simulation as bound time
...
VOID CheckForTermination() {
  ...
  if (zinfo->maxSimTimeNs) {
    uint64_t simNs = zinfo->profSimTime->count(PROF_BOUND) +
                     zinfo->profSimTime->count(PROF_WEAVE);
...
/* This is called by the scheduler at the end of a phase. At that point,
 * zinfo->numPhases has not incremented, so it denotes the END of the current
 * phase
 */
VOID EndOfPhaseActions() {
  ...
  // Done before tick() to avoid deadlock in most cases when entering synced
  // ffwd (can we still deadlock with sleeping threads?)
  ...
  CheckForTermination();
  zinfo->contentionSim->simulatePhase(zinfo->globPhaseCycles +
                                      zinfo->phaseLength);
...
uint32_t TakeBarrier(uint32_t tid, uint32_t cid) {
  ...
  clearCid(tid);  // this is after the sync for a hack needed to make EndOfPhase
                  // reliable
  setCid(tid, newCid);
...
VOID Instruction(INS ins) {
  ...
  // IARG_REG_VALUE, REG_INST_PTR, IARG_END);
  ...
  if (!procTreeNode->isInFastForward() || !zinfo->ffReinstrument) {
    ...
    if (INS_IsMemoryRead(ins)) {
      if (!INS_IsPredicated(ins)) {
        INS_InsertCall(ins, IPOINT_BEFORE, LoadFuncPtr, IARG_FAST_ANALYSIS_CALL,
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, PredLoadFuncPtr,
                       ...
                       IARG_MEMORYREAD_EA, IARG_EXECUTING, IARG_END);
      }
    }
    ...
    if (INS_HasMemoryRead2(ins)) {
      if (!INS_IsPredicated(ins)) {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, PredLoadFuncPtr,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                       IARG_MEMORYREAD2_EA, IARG_EXECUTING, IARG_END);
      }
    }
    ...
    if (INS_IsMemoryWrite(ins)) {
      if (!INS_IsPredicated(ins)) {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, PredStoreFuncPtr,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                       IARG_MEMORYWRITE_EA, IARG_EXECUTING, IARG_END);
      }
    }
    ...
    if (INS_Category(ins) == XED_CATEGORY_COND_BR) {
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)IndirectRecordBranch,
                     IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_INST_PTR,
   ...
   */
  if (INS_IsXchg(ins) && INS_OperandReg(ins, 0) == REG_RCX &&
      INS_OperandReg(ins, 1) == REG_RCX) {
    // info("Instrumenting magic op");
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)HandleMagicOp, IARG_THREAD_ID,
                   IARG_REG_VALUE, REG_ECX, IARG_END);
  }

  if (INS_Opcode(ins) == XED_ICLASS_CPUID) {
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)FakeCPUIDPre, IARG_THREAD_ID,
                   IARG_REG_VALUE, REG_EAX, IARG_REG_VALUE, REG_ECX, IARG_END);
    INS_InsertCall(ins, IPOINT_AFTER, (AFUNPTR)FakeCPUIDPost, IARG_THREAD_ID,
                   IARG_REG_REFERENCE, REG_EAX, IARG_REG_REFERENCE, REG_EBX,
                   IARG_REG_REFERENCE, REG_ECX, IARG_REG_REFERENCE, REG_EDX,
                   IARG_END);
  }

  if (INS_IsRDTSC(ins)) {
    // No pre; note that this also instruments RDTSCP
    INS_InsertCall(ins, IPOINT_AFTER, (AFUNPTR)FakeRDTSCPost, IARG_THREAD_ID,
                   IARG_REG_REFERENCE, REG_EAX, IARG_REG_REFERENCE, REG_EDX,
                   IARG_END);
  }

  if (INS_IsSyscall(ins)) {
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)SyscallEnterUnlocked,
                   IARG_CALL_ORDER, CALL_ORDER_LAST, IARG_THREAD_ID,
...
VOID Trace(TRACE trace, VOID* v) {
  if (!procTreeNode->isInFastForward() || !zinfo->ffReinstrument) {
    ...
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
      ...
      BBL_InsertCall(bbl, IPOINT_BEFORE /*could do IPOINT_ANYWHERE if we redid
                                           load and store simulation in OOO*/
                     ,
                     (AFUNPTR)IndirectBasicBlock, IARG_FAST_ANALYSIS_CALL,
                     IARG_THREAD_ID, IARG_ADDRINT, BBL_Address(bbl), IARG_PTR,
                     bblInfo, IARG_END);
    }
  }
...
// Register hooks to intercept and virtualize time-related vsyscalls and vdso
// syscalls, as they do not show up as syscalls! NOTE: getcpu is also a VDSO
// syscall, but is not patched for now

// vDSO patch data for a single thread in a single context (multiple contexts
// allow signal handlers)
struct VdsoPatchSingleton {
  ...
  VdsoPatchSingleton(ADDRINT _arg0, ADDRINT _arg1, VdsoFunc _func,
                     uint32_t _level)
      : arg0(_arg0), arg1(_arg1), func(_func), level(_level){};
  VdsoPatchSingleton() : arg0(0), arg1(0), func(VdsoFunc(0)), level(0){};
};

// Per-thread vDSO context stack. New stack entries are pushed after a pin
// ContextChange callback (typically a signal), and popped lazily after a return
// from such a context (a SIGRETURN reason in the ContextChange callback).
// Without such a mechanism it would be incorrect to call vDSO functions within
// a signal handler if the prior context were already processing a vDSO
class VdsoPatchStack {
 ...
 public:
  VdsoPatchStack() {
    // Each thread starts with vdso info for level 0 (i.e., not in a signal
    // handler) This is never popped for performance reasons
  ...
  void push(THREADID tid, ADDRINT arg0, ADDRINT arg1, VdsoFunc func,
            uint32_t level) {
  ...
  uint32_t size(THREADID tid) { return (uint32_t)patchStack[tid].size(); };
};
VdsoPatchStack vdsoPatchData;

// Per-thread signal depth info. (0 -> not in a signal handler, n>0 -> in level
// 'n' signal handler)
unsigned int signalStackDepth[MAX_THREADS] = {};

// Analysis functions

// Helper function to laziliy clean up old vdso signal stacks. The alternative
// would be to register these actions for the SIGRETURN callback
...
VOID VdsoEntryPoint(THREADID tid, uint32_t func, ADDRINT arg0, ADDRINT arg1) {
  ...
  if (vdsoDepth < sigStackDepth) {
    ...
    if (vdso.level) {
      // common, in Ubuntu 11.10 several vdso functions jump back to the
      // callpoint info("vDSO function (%d) called from vdso (%d), level %d,
      // skipping", func, vdso.func, vdso.level);
    } else {
      vdso.arg0 = arg0;
      vdso.arg1 = arg1;
...
VOID VdsoRetPoint(THREADID tid, REG* raxPtr) {
  ...
  if (vdso.level == 0) {
    warn(
        "vDSO return without matching call --- did we instrument all the "
        "functions?");
  ...
  vdso.level--;
  if (vdso.level) {
    // info("vDSO return post level %d, skipping ret handling", vdso.level);
    // //common
    return;
  }
...
VOID VdsoInstrument(INS ins) {
  ...
  if (unlikely(insAddr >= vdsoStart && insAddr < vdsoEnd)) {
    if (vdsoEntryMap.find(insAddr) != vdsoEntryMap.end()) {
      ...
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)VdsoEntryPoint,
                     IARG_THREAD_ID, IARG_UINT32, (uint32_t)func,
                     IARG_REG_VALUE, REG_RDI, IARG_REG_VALUE, REG_RSI,
                     IARG_END);
    } else if (INS_IsCall(ins)) {
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)VdsoCallPoint, IARG_THREAD_ID,
                     IARG_END);
    } else if (INS_IsRet(ins)) {
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)VdsoRetPoint, IARG_THREAD_ID,
                     IARG_REG_REFERENCE, REG_RAX /* return val */, IARG_END);
    }
  }
  ...
  if (unlikely(insAddr >= vsyscallStart && insAddr < vsyscallEnd &&
               !vsyscallWarned)) {
    warn(
        "Instrumenting vsyscall page code --- this process executes vsyscalls, "
        "which zsim does not virtualize!");
    vsyscallWarned = true;
  }
}
...
bool activeThreads[MAX_THREADS];  // set in ThreadStart, reset in ThreadFini, we
                                  // need this for exec() (see FollowChild)
bool inSyscall[MAX_THREADS];      // set in SyscallEnter, reset in SyscallExit,
                                  // regardless of state. We MAY need this for
                                  // ContextChange
...
// Per-thread storage to transfer modified syscall arguments between invocations
// of SyscallEnterUnlocked and SyscallEnterLocked. Modified syscall args are
// stored at indexes [0:5], syscall number - at index 6, flag indicating
// modification - at index 7.
static ADDRINT
    modifiedSyscallArgs[MAX_THREADS]
                       [MOD_SYSCALL_INDEXES];  // set in SyscallEnterUnlocked,
                                               // used in SyscallEnterLocked
...
VOID SaveModifiedSyscallArgs(THREADID tid, CONTEXT* ctxtMod,
                             CONTEXT* ctxtOrig) {
  ADDRINT modified = 0;
  SYSCALL_STANDARD std = SYSCALL_STANDARD_IA32E_LINUX;
  if (PIN_GetSyscallNumber(ctxtMod, std) !=
      PIN_GetSyscallNumber(ctxtOrig, std)) {
    modified = 1;
  }
  for (uint32_t argInd = 0; (argInd < MOD_SYSCALL_ARGS_NUM) && !modified;
       argInd++) {
    if (PIN_GetSyscallArgument(ctxtMod, std, argInd) !=
        PIN_GetSyscallArgument(ctxtOrig, std, argInd)) {
      modified = 1;
    }
  }
  if (modified) {
    modifiedSyscallArgs[tid][MOD_SYSCALL_NUMBER_INDEX] =
        PIN_GetSyscallNumber(ctxtMod, std);
    for (uint32_t argInd = 0; argInd < MOD_SYSCALL_ARGS_NUM; argInd++) {
      modifiedSyscallArgs[tid][argInd] =
          PIN_GetSyscallArgument(ctxtMod, std, argInd);
    }
...
VOID WriteModifiedSyscallArgs(THREADID tid, CONTEXT* ctxt,
                              SYSCALL_STANDARD std) {
  assert(std == SYSCALL_STANDARD_IA32E_LINUX);
  if (modifiedSyscallArgs[tid][MOD_SYSCALL_FLAG_INDEX]) {
    PIN_SetSyscallNumber(ctxt, std,
                         modifiedSyscallArgs[tid][MOD_SYSCALL_NUMBER_INDEX]);
    for (uint32_t argInd = 0; argInd < MOD_SYSCALL_ARGS_NUM; argInd++) {
      PIN_SetSyscallArgument(ctxt, std, argInd,
                             modifiedSyscallArgs[tid][argInd]);
    }
...
// Performs action related to SyscallEnter without acquiring the Pin internal
// lock. Need to remove ourselves from running threads in case the syscall is
// blocking. Note: the rationale for having two methods SyscallEnterUnlocked and
// SyscallEnterLocked is descibed at https://github.com/s5z/zsim/issues/57
VOID SyscallEnterUnlocked(THREADID tid, CONTEXT* ctxt) {
  ...
  if (fPtrs[tid].type != FPTR_JOIN && !zinfo->blockingSyscalls) {
    ...
    clearCid(tid);
    zinfo->sched->syscallLeave(
        procIdx, tid, cid, PIN_GetContextReg(ctxt, REG_INST_PTR),
...
VOID SyscallEnterLocked(THREADID tid, CONTEXT* ctxt, SYSCALL_STANDARD std,
                        VOID* v) {
  assert(inSyscall[tid]);
...
VOID SyscallExit(THREADID tid, CONTEXT* ctxt, SYSCALL_STANDARD std, VOID* v) {
  ...
  if (ppa == PPA_USE_JOIN_PTRS) {
    if (!zinfo->blockingSyscalls) {
      ...
      fPtrs[tid] =
          cores[tid]->GetFuncPtrs();  // go back to normal pointers, directly
    }
  ...
  if (fPtrs[tid].type == FPTR_JOIN && procTreeNode->isInFastForward()) {
    ...
    // We are not in the scheduler, and have no cid assigned. So, no need to
    // leave()
    SimThreadFini(tid);
    fPtrs[tid] = GetFFPtrs();
  }
...
VOID ContextChange(THREADID tid, CONTEXT_CHANGE_REASON reason,
  ...
  warn("[%d] ContextChange, reason %s (%d), inSyscall %d, sigLevel: %d", tid,
       reasonStr, info, inSyscall[tid], vdsoPatchData.peek(tid).level);
  if (inSyscall[tid]) {
  ...
  if (reason == CONTEXT_CHANGE_REASON_FATALSIGNAL) {
    ...
    zinfo->sched->queueProcessCleanup(
        procIdx, getpid());  // the scheduler watchdog will remove all our state
                             // when we are really dead
    SimEnd();
  }

  // If this is an issue, we might need to call syscallexit on occasion. I very
  // much doubt it SyscallExit(tid, to, SYSCALL_STANDARD_IA32E_LINUX, nullptr);
  // //NOTE: For now it is safe to do spurious syscall exits, but careful...
}
...
BOOL FollowChild(CHILD_PROCESS childProcess, VOID* userData) {
  ...
  // You can always run process0 = { command = "ls"; startPaused = True;
  // startFastForwarded = True; }; to avoid this
  if (procIdx == 0)
    panic(
        "process0 cannot exec(), it spawns globally needed internal threads "
        "(scheduler and contention); run a dummy process0 instead!");

  // Set up Pin command
  // NOTE: perProcessDir may be active, we don't care much... run in the same
  // dir as parent process NOTE: we recycle our own procIdx on an exec, but
  // fork() changed it so we need to update Pin's command line
...
VOID BeforeFork(THREADID tid, const CONTEXT* ctxt, VOID* arg) {
  ...
  info("Thread %d forking, child procIdx=%d", tid,
...
VOID AfterForkInChild(THREADID tid, const CONTEXT* ctxt, VOID* arg) {
  ...
  char header[64];
  snprintf(header, sizeof(header), "[S %dF] ",
           procIdx);  // append an F to distinguish forked from fork/exec'd
  ...
  info("Forked child (tid %d/%d), PID %d, parent PID %d", tid, PIN_ThreadId(),
       PIN_GetPid(), getppid());

  // Initialize process-local per-thread state, even if ThreadStart does so
  // later
  for (uint32_t i = 0; i < MAX_THREADS; i++) {
    fPtrs[i] = joinPtrs;
...
VOID Fini(int code, VOID* v) {
  ...
  // NOTE: In fini, it appears that info() and writes to stdout in general won't
  // work; warn() and stderr still work fine.
  SimEnd();
}

VOID SimEnd() {
  if (__sync_bool_compare_and_swap(&perProcessEndFlag, 0, 1) ==
      false) {      // failed, note DEPENDS ON STRONG CAS
  ...
  // at this point, we're in charge of exiting our whole process, but we still
  // need to race for the stats
  ...
  (void)lastToFinish;  // make gcc happy; not needed anymore, since proc 0 dumps
                       // stats
  ...
  // Uncomment when debugging termination races, which can be rare because they
  // are triggered by threads of a dying process sleep(5);
    ...
    case ZSIM_MAGIC_OP_ROI_END:
      if (!zinfo->ignoreHooks) {
        ...
        if (procTreeNode->getSyncedFastForward()) {
          ...
          EnterFastForward();
          // If we don't do this, we'll enter FF on the next phase. Which would
          // be OK, except with synced FF we stay in the barrier forever. And
          // deadlock. And the deadlock code does nothing, since we're in FF So,
          // force immediate entry if we're sync-ffwding
          if (procTreeNode->getSyncedFastForward()) {
    ...
    case ZSIM_MAGIC_OP_REGISTER_THREAD:
      if (!zinfo->registerThreads) {
        ...
        if (fPtrs[tid].type == FPTR_NOP) {
          ...
          warn(
              "Thread %d: Treating REGISTER_THREAD magic op as NOP, thread "
              "already registered",
              tid);
        }
      }
...
VOID FakeCPUIDPost(THREADID tid, ADDRINT* eax, ADDRINT* ebx, ADDRINT* ecx,
  ...
  CpuIdRecord val = {eaxIn,        ecxIn,        (uint32_t)-1,
                     (uint32_t)-1, (uint32_t)-1, (uint32_t)-1};
  CpuIdRecord* pos = std::lower_bound(
      cpuid_core2, cpuid_core2 + (sizeof(cpuid_core2) / sizeof(CpuIdRecord)),
      val);
  if (pos->eaxIn > eaxIn) {
  ...
  // info("%x %x : %x %x / %x %x %x %x", eaxIn, ecxIn, pos->eaxIn, pos->ecxIn,
  // pos->eax, pos->ebx, pos->ecx, pos->edx);
  ...
  if (eaxIn == 4) {
    ...
    if (zinfo->numCores > 64)
      eax3126 = 63;  // looked into swarm2.csail (4P Westmere-EX, 80 HTs), it
                     // sets this to 63
  ...
  if (eaxIn == 0x1) {
    ...
    uint32_t apicId =
        (cpu < ncpus) ? MIN(cpu, (uint32_t)255) : 0 /*not scheduled, ffwd?*/;
    ebxOut = (ebxOut & 0xffff) | (siblings << 16) | (apicId << 24);
  }

  // info("[%d] postcpuid, inEax 0x%x, pre 0x%lx 0x%lx 0x%lx 0x%lx", tid, eaxIn,
  // *eax, *ebx, *ecx, *edx); Preserve high bits
  ...
  *reinterpret_cast<uint32_t*>(edx) = pos->edx;
  // info("[%d] postcpuid, inEax 0x%x, post 0x%lx 0x%lx 0x%lx 0x%lx", tid,
  // eaxIn, *eax, *ebx, *ecx, *edx);
}
...
VOID FakeRDTSCPost(THREADID tid, REG* eax, REG* edx) {
  ...
  // info("[t%d/c%d] Virtualizing RDTSC, pre = %x %x (%ld), post = %x %x (%ld)",
  // tid, cid, *edx, *eax, origTSC, hi, lo, curCycle);
...
class SyncEvent : public Event {
 ...
 public:
  ...
  void wait() { futex_lock(&arrivalLock); }
  ...
  void signal() { futex_unlock(&leaveLock); }
};
...
VOID FFThread(VOID* arg) {
  ...
  while (true) {
    ...
    bool locked = futex_trylock_nospin_timeout(&zinfo->ffToggleLocks[procIdx],
                                               5 * BILLION /*5s timeout*/);
    ...
    futex_lock(&zinfo->ffLock);
    if (procTreeNode->isInFastForward()) {
      GetVmLock();  // like a callback. This disallows races on all syscall
                    // instrumentation, etc.
      ...
      info("Pending fast-forward entry, waiting for end of phase (%ld phases)",
           zinfo->numPhases);

      futex_unlock(&zinfo->ffLock);
      syncEv->wait();
      // At this point the thread thet triggered the end of phase is blocked
      // inside of EndOfPhaseActions
      futex_lock(&zinfo->ffLock);
      if (!procTreeNode->isInFastForward()) {
        ...
        info(
            "FF control thread called on end of phase, but someone else "
            "(program?) already entered ffwd");
      }
...
// When firing a debugger was an easy affair, this was not an issue. Now it's
// not so easy, so let's try to at least capture the backtrace and print it out

// Use unlocked output, who knows where this happens.
static EXCEPT_HANDLING_RESULT InternalExceptionHandler(
    THREADID tid, EXCEPTION_INFO* pExceptInfo, PHYSICAL_CONTEXT* pPhysCtxt,
    VOID*) {
  fprintf(stderr, "%s[%d] Internal exception detected:\n", logHeader, tid);
  fprintf(stderr, "%s[%d]  Code: %d\n", logHeader, tid,
          PIN_GetExceptionCode(pExceptInfo));
  fprintf(stderr, "%s[%d]  Address: 0x%lx\n", logHeader, tid,
          PIN_GetExceptionAddress(pExceptInfo));
  ...
  for (uint32_t i = 0; i < size; i++) {
    // For libzsim.so addresses, call addr2line to get symbol info (can't use
    // -rdynamic on libzsim.so because of Pin's linker script) NOTE: May be
    // system-dependent, may not handle malformed strings well. We're going to
    // die anyway, so in for a penny, in for a pound...
...
int main(int argc, char* argv[]) {
  ...
  // This avoids leaving strays running in any circumstances, but may be too
  // heavy-handed with arbitrary process hierarchies. If you ever need this
  // disabled, sim.pinOptions = "-injection child" does the trick
  ...
  if (procIdx == 0 &&
      !gm_isready()) {  // process 0 can exec() without fork()ing first, so we
                        // must check gm_isready() to ensure we don't initialize
                        // twice
    masterProcess = true;
    SimInit(KnobConfigFile.Value().c_str(), KnobOutputDir.Value().c_str(),
            KnobShmid.Value());
  } else {
    while (!gm_isready())
  ...
  if (memcmp(&libzsimAddrs, &zinfo->libzsimAddrs, sizeof(libzsimAddrs)) != 0) {
    panic(
        "libzsim.so address mismatch! text: %p != %p. Perform loader injection "
        "to homogenize offsets!",
        libzsimAddrs.textAddr, zinfo->libzsimAddrs.textAddr);
  }

  // Attach to debugger if needed (master process does so in SimInit, to be able
  // to debug initialization) NOTE: Pin fails to follow exec()'s when gdb is
  // attached. The simplest way to avoid it is to kill the debugger manually
  // before an exec(). If this is common, we could automate it
  ...
  if (!masterProcess)
    procTreeNode->notifyStart();  // masterProcess notifyStart is called in
                                  // init() to avoid races
  assert(procTreeNode->getProcIdx() == (uint32_t)procIdx);  // must be
                                                            // consistent
  ...
  info("Started process, PID %d",
       getpid());  // NOTE: external scripts expect this line, please do not
                   // change without checking first

  // Unless things change substantially, keep this disabled; it causes higher
  // imbalance and doesn't solve large system time with lots of processes.
  ...
  VdsoInit();  // initialized vDSO patching information (e.g., where all the
               // possible vDSO entry points are)
  ...
  // FFwd control
  // OK, screw it. Launch this on a separate thread, and forget about signals...
  // the caller will set a shared memory var. PIN is hopeless with signal
  // instrumentation on multithreaded processes!
  PIN_SpawnInternalThread(FFThread, nullptr, 64 * 1024, nullptr);

  // Start trace-driven or exec-driven sim
  if (zinfo->traceDriven) {
    info("Running trace-driven simulation");
    while (!zinfo->terminationConditionMet &&
           zinfo->traceDriver->executePhase()) {
      // info("Phase done");
      EndOfPhaseActions();
      zinfo->numPhases++;
