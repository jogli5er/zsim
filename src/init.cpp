...
/* zsim should be initialized in a deterministic and logical order, to avoid
 * re-reading config vars all over the place and give a predictable global state
 * to constructors. Ideally, this should just follow the layout of zinfo,
 * top-down.
 */

BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name,
                          uint32_t bankSize, bool isTerminal, uint32_t domain) {
  ...
  if ((1u << setBits) != numSets)
    panic("%s: Number of sets must be a power of two (you specified %d sets)",
          name.c_str(), numSets);

  // Hash function
  HashFamily* hf = nullptr;
  string hashType = config.get<const char*>(
      prefix + "array.hash",
  ...
  if (numHashes) {
    if (hashType == "None") {
      ...
      panic("%s: Invalid value %s on array.hash", name.c_str(),
            hashType.c_str());
    }
  }

  // Replacement policy
  string replType = config.get<const char*>(
      prefix + "repl.type",
  ...
  if (replType == "LRU" || replType == "LRUNoSh") {
  ...
  } else if (replType == "WayPart" || replType == "Vantage" ||
             replType == "IdealLRUPart") {
    ...
    string partMapper =
        config.get<const char*>(prefix + "repl.partMapper", "Core");
    PartMapper* pm = nullptr;
    if (partMapper == "Core") {
      pm = new CorePartMapper(
          zinfo->numCores);  // NOTE: If the cache is not fully shared, trhis
                             // will be inefficient...
      ...
      panic("Invalid repl.partMapper %s on %s", partMapper.c_str(),
            name.c_str());
    }
    ...
    PartitionMonitor* mon = new UMonMonitor(numLines, umonLines, umonWays,
                                            pm->getNumPartitions(), buckets);
    ...
    if (replType == "WayPart") {
      ...
      allocPortion = .85;
      bool smoothTransients =
          config.get<bool>(prefix + "repl.smoothTransients", false);
      prp = new VantageReplPolicy(mon, pm, numLines, assoc,
                                  (uint32_t)(allocPortion * 100), 10, 50,
                                  buckets, smoothTransients);
    }
    rp = prp;

    // Partitioner
    // TODO: Depending on partitioner type, we want one per bank or one per
    // cache.
    Partitioner* p = new LookaheadPartitioner(prp, pm->getNumPartitions(),
                                              buckets, 1, allocPortion);

    // Schedule its tick
    uint32_t interval =
        config.get<uint32_t>(prefix + "repl.interval", 5000);  // phases
  ...
  uint32_t accLat =
      (isTerminal) ? 0
                   : latency;  // terminal caches has no access latency b/c it
                               // is assumed accLat is hidden by the pipeline
  ...
  if (!isTerminal) {
    if (type == "Simple") {
      ...
      uint32_t timingCandidates =
          config.get<uint32_t>(prefix + "timingCandidates", candidates);
      cache = new TimingCache(numLines, cc, array, rp, accLat, invLat, mshrs,
                              tagLat, ways, timingCandidates, domain, name);
    ...
    cache =
        new FilterCache(numSets, numLines, cc, array, rp, accLat, invLat, name);
  }
...
DDRMemory* BuildDDRMemory(Config& config, uint32_t lineSize, uint32_t frequency,
                          uint32_t domain, g_string name,
                          const string& prefix) {
  uint32_t ranksPerChannel =
      config.get<uint32_t>(prefix + "ranksPerChannel", 4);
  uint32_t banksPerRank =
      config.get<uint32_t>(prefix + "banksPerRank", 8);  // DDR3 std is 8
  uint32_t pageSize = config.get<uint32_t>(prefix + "pageSize",
                                           8 * 1024);  // 1Kb cols, x4 devices
  const char* tech = config.get<const char*>(
      prefix + "tech", "DDR3-1333-CL10");  // see cpp file for other techs
  const char* addrMapping = config.get<const char*>(
      prefix + "addrMapping", "rank:col:bank");  // address splitter interleaves
  ...
  // Balances throughput and fairness; 0 -> FCFS / high (e.g., -1) -> pure
  // FR-FCFS
  uint32_t maxRowHits = config.get<uint32_t>(prefix + "maxRowHits", 4);

  // Request queues
  uint32_t queueDepth = config.get<uint32_t>(prefix + "queueDepth", 16);
  uint32_t controllerLatency = config.get<uint32_t>(
      prefix + "controllerLatency", 10);  // in system cycles

  auto mem =
      new DDRMemory(zinfo->lineSize, pageSize, ranksPerChannel, banksPerRank,
                    frequency, tech, addrMapping, controllerLatency, queueDepth,
                    maxRowHits, deferWrites, closedPage, domain, name);
  return mem;
}
...
MemObject* BuildMemoryController(Config& config, uint32_t lineSize,
  ...
  // Latency
  uint32_t latency =
      (type == "DDR") ? -1 : config.get<uint32_t>("sys.mem.latency", 100);
  ...
  if (type == "Simple") {
    ...
    // NOTE: Frequency (in MHz) -- note this is a sys parameter (not sys.mem).
    // There is an implicit assumption of having a single CCT across the system,
    // and we are dealing with latencies in *core* clock cycles
    ...
    uint32_t boundLatency =
        config.get<uint32_t>("sys.mem.boundLatency", latency);
...
CacheGroup* BuildCacheGroup(Config& config, const string& name,
                            bool isTerminal) {
    ...
    if (prefetch_type.compare("Stride") == 0 ||
      ...
      for (uint32_t i = 0; i < prefetchers; i++) {
        ...
        if (prefetch_type.compare("GHB") == 0) {
          ...
          bool monitor_GETS = config.get<bool>(prefix + "monitor_GETS", true);
          bool monitor_GETX = config.get<bool>(prefix + "monitor_GETX", true);
          cg[i][0] = new GhbPrefetcher(
              pfName, ghb_size, index_size, log_distance, degree, target_cache,
              delta_correlat, monitor_GETS, monitor_GETX);
        } else {
          if (!(degree > 0 && degree < 5)) {
            panic("Prefetch degree must be in between 1 and 4");
          }
          cg[i][0] =
              new StreamPrefetcher(pfName, streams, log_distance, degree);
        }
      }
      return cgp;
      ...
      bool monitor_loads =
          config.get<bool>(prefix + "monitorLoads", true);  // really 'GETS'
      bool monitor_stores =
          config.get<bool>(prefix + "monitorStores", true);  // really 'GETX'
      uint32_t degree = config.get<uint32_t>(prefix + "degree", 1);
      bool stride_detection =
          config.get<bool>(prefix + "strideDetection", false);
      ...
      for (uint32_t i = 0; i < prefetchers; i++) {
        ...
        cg[i].emplace_back(new TablePrefetcher(
            full_name, target_cache, monitor_loads, monitor_stores, table_file,
            conf_thresh, prob_thresh, degree, stride_detection));
      }
      return cgp;
      ...
      for (uint32_t i = 0; i < prefetchers; i++) {
        ...
        ss << name << '-' << i;
        g_string full_name(ss.str().c_str());
        cg[i].emplace_back(new NextLinePrefetcher(
            full_name, target_cache, monitor_loads, monitor_stores, degree));
      }
      return cgp;
      ...
      bool new_rr = config.get<bool>(prefix + "new_rr", false);
      bool page_offsets = config.get<bool>(prefix + "page_offsets", false);
      bool scanning_algorithm =
          config.get<bool>(prefix + "scanning_algorithm", false);
      if (target_cache.empty()) {
        panic("Unspecified target cache for prefetcher '%s'", name.c_str());
      }
      cg.resize(prefetchers);
      for (uint32_t i = 0; i < prefetchers; i++) {
        ...
        ss << name << '-' << i;
        g_string full_name(ss.str().c_str());
        cg[i].emplace_back(new BestOffsetPrefetcher(
            full_name, target_cache, monitor_loads, monitor_stores, degree,
            round_max, max_score, init_offset, rr_size, new_rr, page_offsets,
            scanning_algorithm));
      }
      return cgp;
    } else {
  ...
  if (size % banks != 0) {
    panic("%s: banks (%d) does not divide the size (%d bytes)", name.c_str(),
          banks, size);
  }
...
static void InitSystem(Config& config) {
  ...
  unordered_map<string, vector<vector<string>>>
      childMap;  // parent -> children (a parent may have multiple children)

  auto parseChildren = [](string children) {
    // 1st dim: concatenated caches; 2nd dim: interleaved caches
    // Example: "l2-beefy l1i-wimpy|l1d-wimpy" produces [["l2-beefy"],
    // ["l1i-wimpy", "l1d-wimpy"]] If there are 2 of each cache, the final
    // vector will be l2-beefy-0 l2-beefy-1 l1i-wimpy-0 l1d-wimpy-0 l1i-wimpy-1
    // l1d-wimpy-1
  ...
  Network* network =
      (networkFile != "") ? new Network(networkFile.c_str()) : nullptr;
  ...
  for (auto& it : childMap)
    if (!parentMap.count(it.first)) parentlessCacheGroups.push_back(it.first);
  if (parentlessCacheGroups.size() != 1)
    panic("Only one last-level cache allowed, found: %s",
  ...
  // Check single LLC
  if (cMap[llc]->size() != 1)
    panic("Last-level cache %s must have caches = 1, but %ld were specified",
          llc.c_str(), cMap[llc]->size());

  /* Since we have checked for no loops, parent is mandatory, and all parents
   * are checked valid, it follows that we have a fully connected tree finishing
   * at the LLC.
   */
  ...
  for (uint32_t i = 0; i < memControllers; i++) {
    ...
    mems[i] = BuildMemoryController(config, zinfo->lineSize, zinfo->freqMHz,
                                    domain, name);
  }
  ...
  for (const char* grp : cacheGroupNames) {
    ...
    for (auto childVec : childMap[grp]) {
      ...
      childCaches.insert(childCaches.end(), interleavedGroup.begin(),
    ...
    for (uint32_t p = 0; p < parents; p++) {
      ...
      parentsVec.insert(
          parentsVec.end(), parentCaches[p].begin(),
          parentCaches[p].end());  // BaseCache* to MemObject* is a safe cast

      uint32_t childId = 0;
      g_vector<BaseCache*> childrenVec;
      for (uint32_t c = p * childrenPerParent; c < (p + 1) * childrenPerParent;
           c++) {
      ...
      if (printHierarchy) {
        ...
        std::transform(childrenVec.begin(), childrenVec.end(),
                       std::back_inserter(cacheNames),
                       [](BaseCache* c) -> string {
                         string s = c->getName();
                         return s;
                       });
        ...
        info("Hierarchy: %s -> %s", Str(cacheNames).c_str(),
             parentName.c_str());
      }
  ...
  for (const char* grp : cacheGroupNames) {
    if (isTerminal(grp)) {
      ...
      if (banks != 1)
        panic("Terminal cache group %s needs to have a single bank, has %d",
              grp, banks);
    }
  }
  ...
  for (const char* grp : cacheGroupNames)
    if (isTerminal(grp)) assignedCaches[grp] = 0;
  ...
  if (!zinfo->traceDriven) {
    ...
    for (const char* group : coreGroupNames) {
      if (parentMap.count(group))
        panic(
            "Core group name %s is invalid, a cache group already has that "
            "name",
            group);
      ...
      if (type == "Simple") {
        ...
        zinfo->oooDecode =
            true;  // enable uop decoding, this is false by default, must be
                   // true if even one OOO cpu is in the system
      } else if (type == "Null") {
      ...
      if (type != "Null") {
        ...
        for (uint32_t j = 0; j < cores; j++) {
          ...
          if (assignedCaches[icache] >= igroup.size()) {
            panic(
                "%s: icache group %s (%ld caches) is fully used, can't connect "
                "more cores to it",
          ...
          if (type == "Simple") {
            ...
            assert(type == "OOO");
            OOOCore* ocore =
                new (&oooCores[j]) OOOCore(ic, dc, name, core_properties);
    ...
    coreIdx = 0;
    for (const char* group : coreGroupNames)
      for (Core* core : coreMap[group]) zinfo->cores[coreIdx++] = core;
    ...
    for (const char* grp : cacheGroupNames) {
      if (isTerminal(grp)) {
        for (vector<BaseCache*> cv : *cMap[grp]) {
          assert(cv.size() == 1);
          TraceDriverProxyCache* proxy =
              dynamic_cast<TraceDriverProxyCache*>(cv[0]);
          assert(proxy);
    ...
    string retraceFile = config.get<const char*>(
        "sim.retraceFile", "");  // leave empty to not retrace
    zinfo->traceDriver = new TraceDriver(
        traceFile, retraceFile, proxies,
        config.get<bool>("sim.useSkews",
                         true),  // incorporate skews in to playback and
                                 // simulator results, not only the output trace
        config.get<bool>("sim.playPuts", true),
        config.get<bool>("sim.playAllGets", true));
    zinfo->traceDriver->initStats(zinfo->rootStat);
  }

  // Call postInit() on every cache. For most caches this will do nothing, but
  // some may wish to take some action that is only available at this point,
  // such as traversing the memory hierarchy.
  ...
  for (const char* group : cacheGroupNames) {
    ...
    for (vector<BaseCache*>& banks : *cMap[group])
      for (BaseCache* bank : banks) bank->initStats(groupStat);
    zinfo->rootStat->append(groupStat);
  }

  // Initialize event recorders
  // for (uint32_t i = 0; i < zinfo->numCores; i++) eventRecorders[i] = new
  // EventRecorder();
...
static void PostInitStats(bool perProcessDir, Config& config) {
  ...
  if (zinfo->statsPhaseInterval) {
    const char* periodicStatsFilter =
        config.get<const char*>("sim.periodicStatsFilter", "");
    AggregateStat* prStat =
        (!strlen(periodicStatsFilter))
            ? zinfo->rootStat
            : FilterStats(zinfo->rootStat, periodicStatsFilter);
    if (!prStat)
      panic(
          "No stats match sim.periodicStatsFilter regex (%s)! Set interval to "
          "0 to avoid periodic stats",
          periodicStatsFilter);
    zinfo->periodicStatsBackend =
        new HDF5Backend(pStatsFile, prStat, (1 << 20) /* 1MB chunks */,
    ...
    zinfo->eventQueue->insert(
        new PeriodicStatsDumpEvent(zinfo->statsPhaseInterval));
    zinfo->statsBackends->push_back(zinfo->periodicStatsBackend);
  } else {
    zinfo->periodicStatsBackend = nullptr;
  }

  zinfo->eventualStatsBackend = new HDF5Backend(
      evStatsFile, zinfo->rootStat, (1 << 17) /* 128KB chunks */,
  ...
  if (zinfo->maxMinInstrs) {
    ...
    for (uint32_t i = 0; i < zinfo->numCores; i++) {
      ...
      zinfo->eventQueue->insert(
          makeAdaptiveEvent(getInstrs, dumpStats, 0, zinfo->maxMinInstrs,
                            MAX_IPC * zinfo->phaseLength));
    }
  }

  // Convenience stats
  StatsBackend* compactStats = new HDF5Backend(
      cmpStatsFile, zinfo->rootStat,
...
void SimInit(const char* configFile, const char* outputDir, uint32_t shmid) {
  ...
  // Debugging
  // NOTE: This should be as early as possible, so that we can attach to the
  // debugger before initialization.
  ...
  if (zinfo->traceDriven) {
    ...
    // TODO: There is some duplication with the core creation code. This should
    // be fixed eventually.
    uint32_t numCores = 0;
    ...
    for (const char* group : groups) {
      uint32_t cores =
          config.get<uint32_t>(string("sys.cores.") + group + ".cores", 1);
      numCores += cores;
    }

    if (numCores == 0)
      panic(
          "Config must define some core classes in sys.cores; sys.numCores is "
          "deprecated");
    zinfo->numCores = numCores;
    assert(numCores <=
           MAX_THREADS);  // TODO: Is there any reason for this limit?
  }

  zinfo->numDomains = config.get<uint32_t>("sim.domains", 1);
  uint32_t numSimThreads = config.get<uint32_t>(
      "sim.contentionThreads",
      MAX((uint32_t)1,
          zinfo->numDomains / 2));  // gives a bit of parallelism, TODO tune
  ...
  zinfo->statsPhaseInterval =
      config.get<uint32_t>("sim.statsPhaseInterval", 100);
  zinfo->freqMHz = config.get<uint32_t>("sys.frequency", 2000);

  // Maxima/termination conditions
  zinfo->maxPhases = config.get<uint32_t>("sim.maxPhases", 0);
  ...
  zinfo->maxProcEventualDumps =
      config.get<uint32_t>("sim.maxProcEventualDumps", 0);
  zinfo->procEventualDumps = 0;

  zinfo->skipStatsVectors = config.get<bool>("sim.skipStatsVectors", false);
  zinfo->compactPeriodicStats =
      config.get<bool>("sim.compactPeriodicStats", false);
  ...
  if (zinfo->ffReinstrument)
    warn(
        "sim.ffReinstrument = true, switching fast-forwarding on a "
        "multi-threaded process may be unstable");
  ...
  zinfo->eventQueue =
      new EventQueue();  // must be instantiated before the memory hierarchy

  if (!zinfo->traceDriven) {
    // Build the scheduler
    uint32_t parallelism = config.get<uint32_t>(
        "sim.parallelism", 2 * sysconf(_SC_NPROCESSORS_ONLN));
    if (parallelism < zinfo->numCores)
      info("Limiting concurrent threads to %d", parallelism);
    assert(parallelism > 0);  // jeez...

    uint32_t schedQuantum =
        config.get<uint32_t>("sim.schedQuantum", 10000);  // phases
    zinfo->sched = new Scheduler(EndOfPhaseActions, parallelism,
                                 zinfo->numCores, schedQuantum);
  } else {
    zinfo->sched = nullptr;
  }
  ...
  if (zinfo->blockingSyscalls) {
    warn(
        "sim.blockingSyscalls = True, will likely deadlock with multi-threaded "
        "apps!");
  }
  ...
  // Process tree needs this initialized, even though it is part of the memory
  // hierarchy
  ...
  // Process hierarchy
  // NOTE: Due to partitioning, must be done before initializing memory
  // hierarchy
  CreateProcessTree(config);
  zinfo->procArray[0]->notifyStart();  // called here so that we can detect
                                       // end-before-start races

  zinfo->pinCmd =
      new PinCmd(&config, nullptr /*don't pass config file to children --- can
                                     go either way, it's optional*/
                 ,
                 outputDir, shmid);
  ...
  const char* procStatsFilter =
      config.get<const char*>("sim.procStatsFilter", "");
  if (strlen(procStatsFilter)) {
    zinfo->procStats = new ProcStats(
        zinfo->rootStat, FilterStats(zinfo->rootStat, procStatsFilter));
  } else {
  ...
  zinfo->profHeartbeats->init("heartbeats", "Per-process heartbeats",
                              zinfo->lineSize);
