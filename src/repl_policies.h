...
/* Generic replacement policy interface. A replacement policy is initialized by
 * the cache (by calling setTop/BottomCC) and used by the cache array. Usage
 * follows two models:
 * - On lookups, update() is called if the replacement policy is to be updated
 * on a hit
 * - On each replacement, rank() is called with the req and a list of
 * replacement candidates.
 * - When the replacement is done, replaced() is called. (See below for more
 * detail.)
 */
 ...
 * This way, we achieve a simple, single interface that is specialized
 * transparently to each type of array (this code is performance-critical)
 */
#define DECL_RANK_BINDING(T) \
  uint32_t rankCands(const MemReq* req, T cands) { return rank(req, cands); }
#define DECL_RANK_BINDINGS          \
  DECL_RANK_BINDING(SetAssocCands); \
  DECL_RANK_BINDING(ZCands);

/* Legacy support.
 * - On each replacement, the controller first calls startReplacement(),
 * indicating the line that will be inserted; then it calls recordCandidate()
 * for each candidate it finds; finally, it calls getBestCandidate() to get the
 *   line chosen for eviction. When the replacement is done, replaced() is
 * called. The division of getBestCandidate() and replaced() happens because the
 * former is called in preinsert(), and the latter in postinsert(). Note how the
 *   same restrictions on concurrent insertions extend to this class, i.e.
 * startReplacement()/recordCandidate()/ getBestCandidate() will be atomic, but
 * there may be intervening update() calls between getBestCandidate() and
 *   replaced().
 */
class LegacyReplPolicy : public virtual ReplPolicy {
 protected:
  virtual void startReplacement(const MemReq* req) {
  }  // many policies don't need it
...
class LRUReplPolicy : public ReplPolicy {
 ...
 public:
  explicit LRUReplPolicy(uint32_t _numLines)
      : timestamp(1), numLines(_numLines) {
    array = gm_calloc<uint64_t>(numLines);
  }

  ~LRUReplPolicy() { gm_free(array); }

  void update(uint32_t id, const MemReq* req) { array[id] = timestamp++; }

  void replaced(uint32_t id) { array[id] = 0; }
 ...
 private:
  inline uint64_t score(uint32_t id) {  // higher is least evictable
    ...
    //  (2) sharers, and
    //  (3) timestamp
    return (sharersAware ? cc->numSharers(id) : 0) * timestamp +
           array[id] * cc->isValid(id);
  }
};
 ...
 */
template <bool sharersAware>
class VCLLRUReplPolicy : public ReplPolicy {
 protected:
  uint64_t timestamp;  // incremented on each access
  uint32_t numLines;
  uint8_t numWays;
 ...
 public:
  explicit VCLLRUReplPolicy(uint32_t _numLines, std::vector<uint8_t> _waySizes)
      : timestamp(1), numLines(_numLines), numWays(_waySizes.size()) {
    array = gm_calloc<uint64_t>(numLines);
    waySizes = gm_calloc<uint8_t>(numWays);
    std::copy(_waySizes.begin(), _waySizes.end(), waySizes);
  }

  ~VCLLRUReplPolicy() { gm_free(array); }
  ...
  template <typename C>
  inline uint32_t rank(const MemReq* req, C cands) {
    return rank(req, cands, 64, 7);
  }

  template <typename C>
  inline uint32_t rank(const MemReq* req, C cands, uint8_t minSize,
                       uint8_t maxWay) {
    assert(minSize <= 64);
    uint32_t bestCand = -1;
    uint64_t bestScore = (uint64_t)-1L;
    uint32_t end = *cands.begin() + maxWay;
    for (auto ci = cands.begin(); *ci < end; ci.inc()) {
      if (waySizes[*ci % numWays] < minSize) {
        continue;
      }
      uint32_t s = score(*ci);
      bestCand = (s < bestScore) ? *ci : bestCand;
...
// This is VERY inefficient, uses LRU timestamps to do something that in essence
// requires a few bits. If you want to use this frequently, consider a
// reimplementation
class TreeLRUReplPolicy : public LRUReplPolicy<true> {
 ...
 public:
  TreeLRUReplPolicy(uint32_t _numLines, uint32_t _numCands)
      : LRUReplPolicy<true>(_numLines), numCands(_numCands), candIdx(0) {
    candArray = gm_calloc<uint32_t>(numCands);
    if (numCands & (numCands - 1))
      panic("Tree LRU needs a power of 2 candidates, %d given", numCands);
  }

  ~TreeLRUReplPolicy() { gm_free(candArray); }

  void recordCandidate(uint32_t id) { candArray[candIdx++] = id; }
  ...
  uint32_t getBestCandidate() {
    ...
    while (end - start > 1) {
      ...
      uint64_t t1 = 0;
      uint64_t t2 = 0;
      for (uint32_t i = start; i < pivot; i++)
        t1 = MAX(t1, array[candArray[i]]);
    ...
    // for (uint32_t i = 0; i < numCands; i++) printf("%8ld ",
    // array[candArray[i]]); info(" res: %d (%d %ld)", start, candArray[start],
    // array[candArray[start]]);
...
class NRUReplPolicy : public LegacyReplPolicy {
 ...
 public:
  NRUReplPolicy(uint32_t _numLines, uint32_t _numCands)
      : numLines(_numLines), numCands(_numCands), youngLines(0), candIdx(0) {
  ...
  uint32_t getBestCandidate() {
    assert(candIdx > 0);
    return candArray[youngLines %
                     candIdx];  // youngLines used to sort-of-randomize
  }
...
class RandReplPolicy : public LegacyReplPolicy {
 ...
 public:
  explicit RandReplPolicy(uint32_t _numCands)
      : numCands(_numCands), rnd(0x23A5F + (uint64_t)this), candIdx(0) {
    candArray = gm_calloc<uint32_t>(numCands);
  }

  ~RandReplPolicy() { gm_free(candArray); }
  ...
  void replaced(uint32_t id) { candIdx = 0; }
};
...
class LFUReplPolicy : public LegacyReplPolicy {
    ...
    inline bool lessThan(const Rank& other, const uint64_t curTs) const {
      if (!valid && other.valid) {
        ...
        if (sharers == 0 && other.sharers > 0) {
          ...
          uint64_t ownInvFreq =
              (curTs - lfuInfo.ts) /
              lfuInfo.acc;  // inverse frequency, lower is better
          uint64_t otherInvFreq =
              (curTs - other.lfuInfo.ts) / other.lfuInfo.acc;
 ...
 public:
  explicit LFUReplPolicy(uint32_t _numLines)
      : timestamp(1), bestCandidate(-1), numLines(_numLines) {
    array = gm_calloc<LFUInfo>(numLines);
    bestRank.reset();
  }

  ~LFUReplPolicy() { gm_free(array); }

  void update(uint32_t id, const MemReq* req) {
    // ts is the "center of mass" of all the accesses, i.e. the average
    // timestamp
    array[id].ts =
        (array[id].acc * array[id].ts + timestamp) / (array[id].acc + 1);
    array[id].acc++;
    timestamp += 1000;  // have larger steps to avoid losing too much resolution
...
class ProfViolReplPolicy : public T {
 ...
 public:
  ...
  void update(uint32_t id, const MemReq* req) {
    ...
    if (cycle < MAX(accTimes[id].read, accTimes[id].write)) {  // violation
      ...
      if (cycle < MIN(accTimes[id].read, accTimes[id].write)) {  // before both
        readViol = (accTimes[id].read < accTimes[id].write);  // read is closer
                 ...
                 accTimes[id]
                     .read) {  // write, current access, read -> XAR viol
        readViol = true;
      ...
      // Record
      read ? (readViol ? profRAR.inc() : profRAW.inc())
           : (readViol ? profWAR.inc() : profWAW.inc());

      // info("0x%lx viol read %d readViol %d cycles: %ld | r %ld w %ld",
      // req->lineAddr, read, readViol, cycle, accTimes[id].read,
      // accTimes[id].write);
    } else {
      profNoViolAcc.inc();
    }
    ...
    // Record
    if (read)
      accTimes[id].read = MAX(accTimes[id].read, req->cycle);
    else
      accTimes[id].write = MAX(accTimes[id].write, req->cycle);
