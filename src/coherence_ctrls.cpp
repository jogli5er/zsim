...
uint32_t MESIBottomCC::getParentId(Address lineAddr) {
  return CacheBankHash::hash(lineAddr, parents.size());
}

void MESIBottomCC::init(const g_vector<MemObject*>& _parents, Network* network,
                        const char* name) {
...
uint64_t MESIBottomCC::processEviction(Address wbLineAddr, uint32_t lineId,
  ...
  switch (*state) {
    ...
    case S:
    case E: {
      MemReq req = {0 /*no PC*/, wbLineAddr,     PUTS,    selfId,
                    state,       cycle,          &ccLock, *state,
                    srcId,       0 /*no flags*/, 0 /*not a prefetch*/};
      respCycle = parents[getParentId(wbLineAddr)]->access(req);
    } break;
    case M: {
      MemReq req = {0 /*no PC*/, wbLineAddr,     PUTX,    selfId,
                    state,       cycle,          &ccLock, *state,
                    srcId,       0 /*no flags*/, 0 /*not a prefetch*/};
      respCycle = parents[getParentId(wbLineAddr)]->access(req);
    } break;
...
uint64_t MESIBottomCC::processAccess(MemReq req, int32_t lineId, uint64_t cycle,
                                     uint32_t flags) {
  uint64_t respCycle = cycle;
  if (req.prefetch > 0) {
    // This is a prefetch request that does not want to be allocated at this
    // cache level
    if (lineId == -1 || array[lineId] == I) {
      MESIState dummyState = I;
      uint32_t parentId = getParentId(req.lineAddr);
      MemReq newReq = {
          req.pc,  req.lineAddr, GETS,      selfId, &dummyState,      cycle,
          &ccLock, dummyState,   req.srcId, flags,  req.prefetch - 1, req.vAddr,
          req.size};
      uint32_t nextLevelLat = parents[parentId]->access(newReq) - cycle;
  ...
  assert((req.type == GETS) ||
         !(flags & MemReq::SPECULATIVE));  // A prefetch is only GETS even if
                                           // triggered by GETX
  switch (req.type) {
    ...
    case GETS:
      if (*state == I) {
        uint32_t parentId = getParentId(req.lineAddr);
        MemReq newReq = {req.pc, req.lineAddr, GETS,    selfId,    state,
                         cycle,  &ccLock,      *state,  req.srcId, flags,
                         0,      req.vAddr,    req.size};
        uint32_t nextLevelLat = parents[parentId]->access(newReq) - cycle;
        uint32_t netLat = parentRTTs[parentId];
        respCycle += nextLevelLat + netLat;
        if (flags & MemReq::SW_SPECULATIVE) {
    ...
    case GETX:
      if (*state == I || *state == S) {
        ...
        if (*state == I)
          ...
          profGETXMissSM.inc();
        uint32_t parentId = getParentId(req.lineAddr);
        MemReq newReq = {req.pc, req.lineAddr, GETX,    selfId,    state,
                         cycle,  &ccLock,      *state,  req.srcId, flags,
                         0,      req.vAddr,    req.size};
        uint32_t nextLevelLat = parents[parentId]->access(newReq) - cycle;
        uint32_t netLat = parentRTTs[parentId];
        ...
        if (*state == E) {
           ...
           * anyway, and must do it if it is the L1 (it's OK not to transition
           * if L2+, we'll TX on the PUTX or invalidate, but doing it this way
           * minimizes the differences between L1 and L2+ controllers); and on a
           * PUTX, because receiving a PUTX while we're in E indicates the child
           * did a silent transition and now that it is evictiong, it's our turn
           * to maintain M info.
           */
          *state = M;
        }
...
void MESIBottomCC::processWritebackOnAccess(Address lineAddr, uint32_t lineId,
                                            AccessType type) {
  MESIState* state = &array[lineId];
  assert(*state == M || *state == E);
  if (*state == E) {
...
void MESIBottomCC::processInval(Address lineAddr, uint32_t lineId, InvType type,
                                bool* reqWriteback) {
  MESIState* state = &array[lineId];
  assert(*state != I);
  switch (type) {
...
uint64_t MESIBottomCC::processNonInclusiveWriteback(
  ...
  if (!nonInclusiveHack)
    panic("Non-inclusive %s on line 0x%lx, this cache should be inclusive",
          AccessTypeName(type), lineAddr);
...
void MESITopCC::init(const g_vector<BaseCache*>& _children, Network* network,
                     const char* name) {
...
uint64_t MESITopCC::sendInvalidates(Address lineAddr, uint32_t lineId,
  ...
  uint64_t maxCycle = cycle;  // keep maximum cycle only, we assume all invals
  ...
  if (!e->isEmpty()) {
    ...
    if (type == INV) {
      ...
      // TODO: This is kludgy -- once the sharers format is more sophisticated,
      // handle downgrades with a different codepath
      assert(e->exclusive);
...
uint64_t MESITopCC::processEviction(Address wbLineAddr, uint32_t lineId,
                                    bool* reqWriteback, uint64_t cycle,
                                    uint32_t srcId) {
  if (nonInclusiveHack) {
...
uint64_t MESITopCC::processAccess(Address lineAddr, uint32_t lineId,
                                  AccessType type, uint32_t childId,
                                  bool haveExclusive, MESIState* childState,
                                  bool* inducedWriteback, uint64_t cycle,
                                  uint32_t srcId, uint32_t flags) {
  Entry* e = &array[lineId];
  uint64_t respCycle = cycle;
  switch (type) {
    ...
    case GETS:
      ...
      break;
    case GETX:
      assert(haveExclusive);  // the current cache better have exclusive access
                              // to this line

      // If child is in sharers list (this is an upgrade miss), take it out
      if (e->sharers[childId]) {
        assert_msg(!e->isExclusive(),
...
uint64_t MESITopCC::processInval(Address lineAddr, uint32_t lineId,
                                 InvType type, bool* reqWriteback,
                                 uint64_t cycle, uint32_t srcId) {
